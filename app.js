...
async function createTimelineWordPackage(scope, entityName) {
    ...
    for (let i = 0; i < descriptors.length; i++) {
        ...
        images.push({ descriptor: descriptors[i], dataURL: await renderTimelineDescriptorToDataURL(descriptors[i], 'png') });
    }
    const criteria = getTimelineCriteriaSnapshot();
    const html = `<!DOCTYPE html><html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40"><head><meta charset="utf-8"><title>Rapport des graphiques Kobo</title><style>@page{size:A4 landscape;margin:1.2cm}body{font-family:Calibri,Arial,sans-serif;color:#243447}h1{color:#1f4e78}h2{color:#2f5597;border-bottom:1px solid #9fbad0;padding-bottom:6px}.criteria{border-collapse:collapse;width:100%;margin-bottom:20px}.criteria th,.criteria td{border:1px solid #cbd5e1;padding:6px;text-align:left}.criteria th{background:#eaf2f8;width:38%}.chart{page-break-after:always;margin:20px 0}.chart img{max-width:100%;height:auto;border:1px solid #d7dde5}.caption{font-size:10pt;color:#667085;margin:6px 0 12px}</style></head><body><h1>${timelineEscapeHtml(TITRE_PLATEFORME)}</h1><h2>Critères de recherche et d’affichage</h2>${timelineCriteriaHtml(criteria)}${images.map(item => `<section class="chart"><h2>${timelineEscapeHtml(item.descriptor.title)}</h2><p class="caption">${timelineEscapeHtml(criteria.granularite)} · ${timelineEscapeHtml(criteria.type_donnees)} · ${timelineEscapeHtml(criteria.organisation)}</p><img src="${item.dataURL}" alt="${timelineEscapeHtml(item.descriptor.title)}"></section>`).join('')}</body></html>`;
    const blob = new Blob(['\ufeff', html], { type: 'application/msword' });
    const label = entityName ? timelineSafeFilename(entityName) : String(scope).toLowerCase();
    return { blob, filename: `rapport_graphiques_${label}_${timelineTimestamp()}.doc` };
}
...
async function exportAdvancedSingleWord(chartKey) {
    try {
        ...
        const criteria = getTimelineCriteriaSnapshot();
        const html = `<!DOCTYPE html><html xmlns:w="urn:schemas-microsoft-com:office:word"><head><meta charset="utf-8"><style>@page{size:A4 landscape;margin:1.2cm}body{font-family:Calibri,Arial}table{border-collapse:collapse;width:100%}th,td{border:1px solid #ccd5df;padding:6px;text-align:left}th{background:#eaf2f8}.chart{margin-top:20px}.chart img{max-width:100%;height:auto}</style></head><body><h1>${timelineEscapeHtml(descriptor.title)}</h1>${timelineCriteriaHtml(criteria)}<div class="chart"><img src="${dataURL}" alt="${timelineEscapeHtml(descriptor.title)}"></div></body></html>`;
        const filename = `${timelineSafeFilename(descriptor.title)}_${timelineTimestamp()}.doc`;
        downloadFile(new Blob(['\ufeff', html], { type: 'application/msword' }), filename);
