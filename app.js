...
async function fetchData() {
    ...
    try {
        ...
        const fetchUrls = [koboUrl]; // Aucun proxy CORS public : confidentialité préservée.
...
async function fetchData() {
    ...
    try {
        ...
        if (!fetchSuccess) {
            throw new Error("Connexion directe KoboToolbox impossible. La version active utilise le service sécurisé Cloudflare Workers pour les assets privés ; vous pouvez aussi importer un fichier JSON.");
        }
...
const KOBO_ATTACHMENT_PROXY_STORAGE_KEY = 'pma_kobo_secure_attachment_proxy_url';
...
const KOBO_SOURCE_DATA_API = `https://kf.kobotoolbox.org/api/v2/assets/${KOBO_SOURCE_ASSET_UID}/data/`;
...
function buildSecureKoboApiProxyUrl(targetUrl) {
    const proxyBase = typeof getSecureKoboAttachmentProxyUrl === 'function'
        ? getSecureKoboAttachmentProxyUrl()
...
async function fetchKoboJsonWithFallback(url) {
    ...
    // 1) Tentative directe : utile uniquement si l'asset autorise explicitement
    //    AnonymousUser à consulter les soumissions.
    ...
    // 2) Pour une API Kobo v2 protégée, le token ne doit JAMAIS être mis dans
    //    le JavaScript GitHub Pages. On passe donc par le Worker Cloudflare sécurisé.
    const proxiedUrl = buildSecureKoboApiProxyUrl(cleanUrl);
    if (proxiedUrl) {
        try {
            ...
            if (!response.ok) {
                let detail = '';
                try { detail = (await response.text()).slice(0, 400); } catch (_) {}
                throw new Error(`Service sécurisé HTTP ${response.status}${detail ? ' — ' + detail : ''}`);
            }
            return await response.json();
        } catch (proxyError) {
            throw new Error(`Accès KoboToolbox authentifié impossible via le service sécurisé : ${proxyError.message || proxyError}`);
        }
    }

    if (directStatus === 401 || directStatus === 403) {
        throw new Error(`HTTP ${directStatus} — KoboToolbox exige une authentification API pour cet asset. Configurez le service sécurisé Cloudflare Workers dans la zone « Service sécurisé API KoboToolbox », puis cliquez sur « Tester » et « Rafraîchir les données ». Vous pouvez aussi utiliser temporairement la source Fichier JSON.`);
    }
    throw new Error(`Connexion KoboToolbox impossible${directError ? ' : ' + (directError.message || directError) : ''}. Si l'asset est privé, configurez le service sécurisé Cloudflare Workers.`);
}
...
function getSecureKoboAttachmentProxyUrl() {
    const input = document.getElementById('kobo-secure-attachment-service-url');
    const fromInput = String(input?.value || '').trim();
    if (fromInput) return fromInput;
    try { return String(localStorage.getItem(KOBO_ATTACHMENT_PROXY_STORAGE_KEY) || '').trim(); }
    catch (_) { return ''; }
}

function updateSecureKoboAttachmentServiceStatus() {
    const status = document.getElementById('kobo-secure-attachment-service-status');
    if (!status) return;
    const proxyUrl = getSecureKoboAttachmentProxyUrl();
    status.innerHTML = proxyUrl
        ? `<span class="text-success"><i class="fas fa-shield-alt"></i> <strong>Cloudflare Workers sécurisé configuré pour l’API Kobo :</strong> <code>${escapeRealisationHtml(proxyUrl)}</code>. Le token Kobo reste uniquement côté serveur. Ce service peut authentifier le téléchargement des données ET des pièces jointes. Utilisez « Tester » avant une synchronisation ou une sauvegarde Base64.</span>`
        : '<span class="text-warning"><i class="fas fa-cloud"></i> <strong>Cloudflare Workers Free est le service gratuit recommandé par défaut, mais il n’est pas encore configuré.</strong> KoboToolbox peut répondre HTTP 401/403 pour les données et les pièces jointes privées. Une URL ne peut pas être universelle car le Worker doit contenir votre propre token Kobo comme secret côté serveur.</span>';
}

window.clearSecureKoboAttachmentProxyUrl = function() {
    const input = document.getElementById('kobo-secure-attachment-service-url');
    if (input) input.value = '';
    try { localStorage.removeItem(KOBO_ATTACHMENT_PROXY_STORAGE_KEY); } catch (_) {}
    const test = document.getElementById('kobo-secure-attachment-service-test');
    if (test) test.innerHTML = '';
    updateSecureKoboAttachmentServiceStatus();
};

async function parseSecureServiceJson(response) {
    try { return await response.clone().json(); } catch (_) { return null; }
}

window.testSecureKoboAttachmentService = async function() {
    const test = document.getElementById('kobo-secure-attachment-service-test');
    const button = document.getElementById('test-kobo-secure-service-btn');
    const proxyUrl = getSecureKoboAttachmentProxyUrl();
    if (!proxyUrl) {
        if (test) test.innerHTML = '<span class="text-danger"><i class="fas fa-times-circle"></i> Aucun Worker configuré. Déployez d’abord le Worker gratuit fourni dans le ZIP, puis saisissez son URL HTTPS.</span>';
        return false;
    }
    if (button) button.disabled = true;
    if (test) test.innerHTML = '<span class="text-primary"><i class="fas fa-spinner fa-spin"></i> Test du Worker Cloudflare et de l’authentification Kobo…</span>';
    try {
        const healthUrl = new URL(proxyUrl, window.location.href);
        healthUrl.searchParams.set('health', '1');
        const healthResponse = await fetch(healthUrl.href, { cache: 'no-store', credentials: 'omit' });
        const health = await parseSecureServiceJson(healthResponse);
        if (!healthResponse.ok || !health?.ok) {
            throw new Error(health?.error || `Worker indisponible — HTTP ${healthResponse.status}`);
        }
        if (!health.token_configured) throw new Error('Le Worker répond, mais KOBO_API_TOKEN n’est pas configuré comme secret côté serveur.');

        // Test prioritaire de l'accès authentifié aux DONNÉES, utile précisément lorsqu'un HTTP 403
        // empêche le tableau de bord de charger la base avant même de disposer de pièces jointes.
        const dataProbeTarget = `${KOBO_SOURCE_DATA_API}?limit=1&_t=${Date.now()}`;
        const dataProbeUrl = buildSecureKoboAttachmentProxyUrl(proxyUrl, dataProbeTarget);
        const dataProbeResponse = await fetch(dataProbeUrl, { cache: 'no-store', credentials: 'omit', headers: { 'Accept': 'application/json' } });
        if (!dataProbeResponse.ok) {
            let detail = ''; try { detail = (await dataProbeResponse.text()).slice(0, 300); } catch (_) {}
            throw new Error(`Le Worker répond, mais l’accès authentifié aux données Kobo échoue — HTTP ${dataProbeResponse.status}${detail ? ' — ' + detail : ''}`);
        }
        const dataProbeBody = await dataProbeResponse.json();
        const dataProbeRows = Array.isArray(dataProbeBody) ? dataProbeBody : (Array.isArray(dataProbeBody?.results) ? dataProbeBody.results : []);

        const rows = Array.isArray(window.allData) ? window.allData : (Array.isArray(window.koboData) ? window.koboData : []);
        const sample = collectKoboAttachments(rows)[0];
        if (!sample) {
            if (test) test.innerHTML = `<span class="text-success"><i class="fas fa-check-circle"></i> <strong>TEST API RÉUSSI.</strong> Service : ${escapeRealisationHtml(health.service || 'Cloudflare Workers')} · token serveur configuré · accès authentifié aux données Kobo validé (${dataProbeRows.length.toLocaleString('fr-FR')} ligne(s) de test). Aucun fichier joint actuellement chargé pour effectuer le test binaire de bout en bout.</span>`;
            return true;
        }

        const probeUrl = buildSecureKoboAttachmentProxyUrl(proxyUrl, sample.url);
        const probeResponse = await fetchKoboAttachmentResponse(probeUrl, { credentials: 'omit' }, 2);
        const contentType = probeResponse.headers.get('content-type') || 'fichier binaire';
        const probeBlob = await probeResponse.blob();
        if (!probeBlob.size) throw new Error('Le Worker a répondu mais la pièce jointe test est vide.');
        if (test) test.innerHTML = `<span class="text-success"><i class="fas fa-check-circle"></i> <strong>TEST COMPLET RÉUSSI.</strong> Cloudflare Worker accessible, secret Kobo configuré, données Kobo accessibles et pièce jointe Kobo récupérée avec authentification (${probeBlob.size.toLocaleString('fr-FR')} octets · ${escapeRealisationHtml(contentType)}). La sauvegarde Base64 peut viser 100 % des pièces jointes accessibles.</span>`;
        return true;
    } catch (error) {
        if (test) test.innerHTML = `<span class="text-danger"><i class="fas fa-times-circle"></i> <strong>ÉCHEC DU TEST :</strong> ${escapeRealisationHtml(String(error?.message || error))}</span>`;
        return false;
    } finally {
...
window.saveSecureKoboAttachmentProxyUrl = function() {
    const input = document.getElementById('kobo-secure-attachment-service-url');
    ...
    if (value) {
        try {
            ...
            localStorage.setItem(KOBO_ATTACHMENT_PROXY_STORAGE_KEY, value);
        } catch (error) {
            alert('Veuillez saisir une URL HTTPS valide pour le service sécurisé.');
            return;
        }
    } else {
        try { localStorage.removeItem(KOBO_ATTACHMENT_PROXY_STORAGE_KEY); } catch (_) {}
...
function loadSecureKoboAttachmentProxyUrl() {
    const input = document.getElementById('kobo-secure-attachment-service-url');
    if (!input) return;
    try { input.value = localStorage.getItem(KOBO_ATTACHMENT_PROXY_STORAGE_KEY) || ''; } catch (_) {}
    updateSecureKoboAttachmentServiceStatus();
}

document.addEventListener('DOMContentLoaded', loadSecureKoboAttachmentProxyUrl);

function buildSecureKoboAttachmentProxyUrl(proxyBaseUrl, originalDownloadUrl) {
    const proxyUrl = new URL(proxyBaseUrl, window.location.href);
    proxyUrl.searchParams.set('url', originalDownloadUrl);
...
function updateKoboJsonExportModeUi() {
    ...
    const servicePanel = document.getElementById('kobo-secure-attachment-service-panel');
    if (servicePanel) servicePanel.classList.toggle('d-none', mode !== 'complete');
    if (mode === 'complete') updateSecureKoboAttachmentServiceStatus();
...
async function fetchKoboAttachmentAsBase64(entry) {
    ...
    const secureProxy = getSecureKoboAttachmentProxyUrl();
    const candidates = [];
    if (secureProxy) {
        candidates.push({
            label: 'service sécurisé',
            url: buildSecureKoboAttachmentProxyUrl(secureProxy, directUrl),
            options: { credentials: 'omit' }
        });
    }
    candidates.push({
        label: 'accès direct KoboToolbox',
        url: directUrl,
    ...
    for (const candidate of candidates) {
        try {
            const response = await fetchKoboAttachmentResponse(candidate.url, candidate.options, 3);
            ...
            return {
                ...
                retrieval_method: candidate.label,
                encoding: 'base64',
            ...
            lastError = error;
            console.warn(`Téléchargement pièce jointe impossible via ${candidate.label}:`, error);
            if (Number(error?.httpStatus || 0) === 403 && candidate.label === 'accès direct KoboToolbox' && !secureProxy) {
                lastError = new Error('HTTP 403 — pièce jointe Kobo protégée. Configurez le service sécurisé d’authentification des pièces jointes pour obtenir une sauvegarde autonome complète.');
...
` +
        (secureProxy
            ? `Service sécurisé configuré : les pièces jointes protégées seront récupérées via le backend authentifié.

`
            : `ATTENTION : aucun service sécurisé n’est configuré. Les pièces jointes protégées peuvent échouer avec HTTP 403 ; dans ce cas le fichier sera explicitement marqué PARTIEL.

`) +
        `Une sauvegarde ne sera déclarée COMPLÈTE que si 100 % des pièces jointes sont incorporées. Continuer ?`
    );
    if (!answer) return;
...
` +
                `Ce fichier NE SERA PAS nommé COMPLETE_BASE64. Il sera explicitement nommé PARTIAL_BASE64 et contiendra le détail des échecs dans attachment_errors.

` +
                `Voulez-vous télécharger malgré tout cette sauvegarde PARTIELLE ?`
            );
            if (!downloadPartial) {
                if (progress) progress.innerHTML = `<span class="text-warning"><strong>Export partiel annulé :</strong> ${embedded.length.toLocaleString('fr-FR')} / ${attachments.length.toLocaleString('fr-FR')} pièce(s) jointe(s) seulement avaient été récupérées. Configurez le service sécurisé puis relancez l’export pour viser 100 %.</span>`;
                return;
            }
        }
        ...
        const payload = await buildVerifiedBackupEnvelope(allData, mode, {
            usage: {
                primary_role: isComplete
                    ...
                    : 'Sauvegarde Base64 PARTIELLE : données Kobo complètes mais certaines pièces jointes binaires n’ont pas pu être incorporées',
                power_bi_compatible: true,
            ...
            statistics: {
                ...
                secure_attachment_service_configured: Boolean(secureProxy)
            },
        ...
        if (progress) {
            progress.innerHTML = isComplete
                ? `<span class="text-success"><strong>✅ SAUVEGARDE AUTONOME COMPLÈTE :</strong> ${embedded.length.toLocaleString('fr-FR')} / ${attachments.length.toLocaleString('fr-FR')} pièce(s) jointe(s) incorporée(s) en Base64. Aucune pièce jointe manquante.</span>`
                : `<span class="text-danger"><strong>⚠️ SAUVEGARDE PARTIELLE — NON COMPLÈTE :</strong> ${embedded.length.toLocaleString('fr-FR')} / ${attachments.length.toLocaleString('fr-FR')} pièce(s) jointe(s) incorporée(s), ${failures.length.toLocaleString('fr-FR')} absente(s). Le fichier a été nommé <code>${escapeRealisationHtml(filename)}</code> et les erreurs sont détaillées dans <code>attachment_errors</code>.${secureProxy ? '' : ' Configurez le service sécurisé pour récupérer les pièces jointes Kobo protégées.'}</span>`;
        }
    } finally {
