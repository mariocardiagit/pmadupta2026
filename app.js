...
function syncRealisationChartControls() {
    const chartType = document.getElementById('real-chart-type')?.value || 'line';
    ...
    if (help) {
        ...
        help.innerHTML = isGantt
            ? '<i class="fas fa-info-circle"></i> <strong>Diagramme de périodes (Gantt)</strong> : chaque barre commence à la « Date debut realisation dans om missionnaire » et se termine à la « Date fin realisation dans om missionnaire ». La valeur est écrite dans la barre ou à son extrémité. Le mode cumulé est désactivé pour préserver une ligne par réalisation.'
...
function createGroupedRealisationGanttChart(level, canvas, stage, records, settings, axisStart, axisEnd) {
    ...
    stage.style.minWidth = `${Math.max(1050, 760 + Math.max(0, rows.length - 8) * 24)}px`;
    stage.style.height = `${Math.min(1500, Math.max(470, 150 + rows.length * 42))}px`;
    const chart = new Chart(canvas.getContext('2d'), {
        type: 'bar',
...
function createIndividualRealisationGanttCharts(level, container, records, settings, axisStart, axisEnd) {
    ...
    ranked.forEach(([entity, total], entityIndex) => {
        ...
        card.innerHTML = `<div class="realisation-individual-heading"><strong>${escapeRealisationHtml(entity)}</strong><span>${entityRows.length} mission(s) · Total : ${Number(total).toLocaleString('fr-FR')}</span></div><div class="realisation-individual-canvas realisation-gantt-individual-canvas"><canvas></canvas></div>`;
        container.appendChild(card);
        const color = getSubmissionTimelineColor(entityIndex, 1);
        const dataset = {
            label: entity,
        ...
        holder.style.height = `${Math.min(850, Math.max(330, 135 + entityRows.length * 45))}px`;
        const chart = new Chart(canvas.getContext('2d'), {
            type: 'bar',
