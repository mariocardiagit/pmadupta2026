...
function getSubmissionLineEndLabelLayout(canvas, datasets) {
    ...
    if (ctx) {
        ...
        (datasets || []).forEach(dataset => {
            ...
            if (label) maxWidth = Math.max(maxWidth, ctx.measureText(label).width);
        });
        ctx.restore();
    } else {
        maxWidth = (datasets || []).reduce((max, dataset) => {
            return Math.max(max, String(dataset?.label || '').trim().length * 7.2);
        }, 0);
    }
    return {
        rightPadding: Math.max(190, Math.min(520, Math.ceil(maxWidth + 42))),
        minGap: 24,
        boxHeight: 20,
        fontSize: 12
    };
...
const submissionLineEndLabelsPlugin = {
    ...
    afterDatasetsDraw(chart, args, pluginOptions) {
        ...
        const fontSize = Math.max(10, Number(pluginOptions.fontSize) || 12);
        const boxHeight = Math.max(18, Number(pluginOptions.boxHeight) || 20);
        let minGap = Math.max(boxHeight + 3, Number(pluginOptions.minGap) || 24);
        ...
        datasets.forEach((dataset, datasetIndex) => {
            ...
            if (!point) return;
            labels.push({
                text: String(dataset.label || '').trim(),
        ...
        const topBound = chartArea.top + boxHeight / 2 + 4;
        const bottomBound = chartArea.bottom - boxHeight / 2 - 4;
        const availableHeight = Math.max(0, bottomBound - topBound);
        if (labels.length > 1 && (labels.length - 1) * minGap > availableHeight) {
            minGap = Math.max(boxHeight + 1, availableHeight / (labels.length - 1));
        }

        for (let i = 0; i < labels.length; i++) {
            labels[i].y = Math.max(labels[i].desiredY, topBound);
            if (i > 0) labels[i].y = Math.max(labels[i].y, labels[i - 1].y + minGap);
        }
        if (labels[labels.length - 1].y > bottomBound) {
            const shift = labels[labels.length - 1].y - bottomBound;
        ...
        if (labels[0].y < topBound) {
            const shift = topBound - labels[0].y;
            labels.forEach(label => { label.y += shift; });
        }
        ...
        labels.forEach(item => {
            ...
            const boxX = Math.min(desiredX, chart.width - boxW - 4);
            ...
            ctx.fillRect(boxX, item.y - boxHeight / 2, boxW, boxHeight);
            ctx.strokeStyle = item.color;
            ctx.lineWidth = 1;
            ctx.strokeRect(boxX, item.y - boxHeight / 2, boxW, boxHeight);
        ...
        options: {
            ...
            plugins: {
                submissionLineEndLabelsPlugin: {
                    ...
                    boxHeight: endLabelLayout.boxHeight,
...
function calculateStableChartDimensions(chart, canvas, host, stage) {
    ...
    if (profile.circular || profile.radar) {
        ...
        if (profile.pointCount > 180 || profile.datasetCount > 9) {
