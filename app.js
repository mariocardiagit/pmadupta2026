...
function syncRealisationChartControls() {
    ...
    if (help) {
        ...
        help.innerHTML = isGantt
            ? '<i class="fas fa-info-circle"></i> <strong>Diagramme de périodes (Gantt)</strong> : chaque barre commence à la « Date debut realisation dans om missionnaire » et se termine à la « Date fin realisation dans om missionnaire ». La valeur est écrite dans la barre ou à son extrémité. Les barres de défilement horizontale et verticale apparaissent automatiquement lorsque la chronologie ou le nombre de missions dépasse la zone visible. Le mode cumulé est désactivé pour préserver une ligne par réalisation.'
...
function getRealisationGanttStageWidth(axisStart, axisEnd, granularity, individual = false) {
    ...
    const minimum = individual ? 1050 : 1180;
    return Math.min(18000, Math.max(minimum, base + units * pixelsPerUnit));
}
...
function buildRealisationGanttOptions(settings, axisStart, axisEnd, rowCount, individual = false) {
    return {
        responsive: true,
        maintainAspectRatio: false,
        normalized: true,
        ...
        layout: { padding: { right: 185, top: 12, bottom: 8, left: 6 } },
        plugins: {
            legend: { display: false },
        ...
        scales: {
            x: {
                ...
                afterBuildTicks: scale => {
                    ...
                    const values = new Set(scale.ticks.map(tick => Number(tick.value)));
                    if (!values.has(minimum)) scale.ticks.unshift({ value: minimum });
                    if (!values.has(maximum)) scale.ticks.push({ value: maximum });
                    scale.ticks.sort((a, b) => Number(a.value) - Number(b.value));
                },
                ticks: {
                    maxTicksLimit: getRealisationGanttTickLimit(settings.granularity),
...
function createGroupedRealisationGanttChart(level, canvas, stage, records, settings, axisStart, axisEnd) {
    ...
    const stageWidth = getRealisationGanttStageWidth(axisStart, axisEnd, settings.granularity, false);
    ...
    const chart = new Chart(canvas.getContext('2d'), {
        ...
        options: buildRealisationGanttOptions(settings, axisStart, axisEnd, rows.length, false)
    });
...
function createIndividualRealisationGanttCharts(level, container, records, settings, axisStart, axisEnd) {
    ...
    ranked.forEach(([entity, total], entityIndex) => {
        ...
        const stageWidth = getRealisationGanttStageWidth(axisStart, axisEnd, settings.granularity, true);
        ...
        const chart = new Chart(canvas.getContext('2d'), {
            ...
            plugins: [realisationGanttLabelsPlugin],
            options: buildRealisationGanttOptions(settings, axisStart, axisEnd, entityRows.length, true)
        });
...
async function buildRealisationSharePackage(refs, prefix, includeCriteria = false) {
    ...
    if (validRefs.length === 1) {
        const blob = realisationDataUrlToBlob(validRefs[0].chart.toBase64Image('image/png', 1));
        return { blob, filename: `${sanitizeRealisationFilename(prefix)}_${stamp}.png`, type: 'image/png' };
    }
    if (typeof JSZip === 'undefined') {
        const blob = realisationDataUrlToBlob(validRefs[0].chart.toBase64Image('image/png', 1));
        return { blob, filename: `${sanitizeRealisationFilename(prefix)}_${stamp}.png`, type: 'image/png' };
    ...
    validRefs.forEach((ref, index) => {
        const base64 = ref.chart.toBase64Image('image/png', 1).split(',')[1];
        zip.file(`${String(index + 1).padStart(2, '0')}_${sanitizeRealisationFilename(ref.name || `graphique_${index + 1}`)}.png`, base64, { base64: true });
    });
    if (includeCriteria) {
        const settings = getRealisationTimelineSettings();
        zip.file('criteres.json', JSON.stringify(settings, (key, value) => value instanceof Date ? formatISODateUTC(value) : value, 2));
    ...
    const blob = await zip.generateAsync({ type: 'blob', compression: 'DEFLATE' });
    return { blob, filename: `${sanitizeRealisationFilename(prefix)}_${stamp}.zip`, type: 'application/zip' };
}
