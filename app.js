...
function updateRealisationComparisonDimensionUi(scope) {
    ...
    if (problemText) problemText.innerHTML = `La comparaison rapproche uniquement des entités qui partagent la <strong>même valeur de la dimension « ${config.label} »</strong>. Deux valeurs différentes de cette dimension ne sont jamais comparées entre elles.`;
}
...
function renderRealisationComparisonScatter(scope, records, analysis) {
    ...
    realisationComparisonCharts[scopeKey].scatter = new Chart(canvas.getContext('2d'), {
        ...
        options: {
            ...
            plugins: {
                title: { display: true, text: `Comparaison numérique des réalisations — ${scopeKey} — ${config.label}`, font: { size: 15, weight: 'bold' } },
            ...
            scales: {
                ...
                y: { type: 'linear', min: -.5, max: Math.max(.5, dimensions.length - .5), reverse: true, title: { display: true, text: config.label, font: { weight: 'bold' } }, ticks: { stepSize: 1, autoSkip: false, callback: value => { const label = dimensions[Math.round(value)]?.[1] || ''; return label.length > 58 ? `${label.slice(0, 55)}…` : label; } }, grid: { color: 'rgba(70, 100, 82, .10)' } }
            }
        }
    });
}
...
function renderRealisationComparisonScatterSummaryTable(scope, records, analysis) {
    const scopeKey = getRealisationComparisonScopeKey(scope);
    const key = scopeKey.toLowerCase();
    const settings = getRealisationComparisonSettings(scopeKey);
    const config = getRealisationComparisonDimensionConfig(settings.dimension);
    const tbody = document.getElementById(`real-compare-scatter-table-${key}`);
    const countNode = document.getElementById(`real-compare-scatter-table-count-${key}`);
    ...
    if (dimensionHeader) dimensionHeader.textContent = config.label;
...
function initializeRealisationComparisonEvents() {
    ...
    document.querySelectorAll('[data-real-compare-algo-tab]').forEach(button => {
        ...
        button.addEventListener('shown.bs.tab', () => {
            ...
            if (!['DREN', 'CISCO', 'ZAP'].includes(scope) || !['kmeans', 'jenks', 'dbscan'].includes(algorithm)) return;
...
/* ======================================================================
   ...
   2) AllOrigins ; 3) CORSPROXY.io ; 4) CORS.lol.
...
const KOBO_PUBLIC_PROXY_PROVIDERS = [
    ...
    {
        // IMPORTANT : l'ancienne syntaxe `https://corsproxy.io/?<url>` peut
        // maintenant répondre HTTP 403. La syntaxe documentée actuelle utilise
        // explicitement le paramètre `url=`.
        name: 'CORSPROXY.io',
...
// la chaîne robuste Direct KoboToolbox -> AllOrigins -> CORSPROXY.io -> CORS.lol.
...
//   2) AllOrigins
//   3) CORSPROXY.io (syntaxe actuelle ?url=)
//   4) CORS.lol
//
