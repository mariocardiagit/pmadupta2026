...
function extractMissionnaireMatriculesFromValue(value) {
    const found = [];
    const collect = item => {
        ...
        const text = cleanSpaces(item);
        const compact = text.replace(/[\s.-]/g, '');
        if (/^(?:\d{6}|\d{12})$/.test(compact)) {
            found.push(compact);
            return;
        }

        // Format d'affichage du tableau :
        // « 1 | 999999 | Nom | URL ; 2 | 831204 | Nom | URL ».
        const pipePattern = /(?:^|;)\s*(?:\d+\s*\|\s*)?(\d{6}|\d{12})(?=\s*\|)/g;
        let match;
        while ((match = pipePattern.exec(text)) !== null) found.push(match[1]);

        // Repli pour une valeur textuelle de champ Matricule/CIN contenant
        // plusieurs identifiants séparés par des espaces ou des ponctuations.
        if (!found.length) {
            const standalone = text.match(/(?<!\d)(?:\d{12}|\d{6})(?!\d)/g) || [];
            found.push(...standalone);
        }
    };
    collect(value);
    return [...new Set(found.map(v => String(v).replace(/[\s.-]/g, '')).filter(v => /^(?:\d{6}|\d{12})$/.test(v)))];
}
...
function extractMatricules(row) {
    ...
    for (const key in (row || {})) {
        ...
        const value = row[key];
        const lowKey = normalizeKoboSearchKey(key);
        ...
        if (Array.isArray(value)) {
            value.forEach(item => {
                ...
                Object.entries(item).forEach(([subKey, subValue]) => {
                    const lowSub = normalizeKoboSearchKey(subKey);
                    if (lowSub.includes('matricule') || lowSub === 'cin' || lowSub.endsWith('_cin')) {
                        mats.push(...extractMissionnaireMatriculesFromValue(subValue));
                        identified = true;
                    }
                });
                if (!identified) {
                    Object.values(item).forEach(subValue => mats.push(...extractMissionnaireMatriculesFromValue(subValue)));
                }
            });
        } else if (lowKey.includes('matricule') || lowKey === 'cin' || lowKey.endsWith('_cin')) {
            mats.push(...extractMissionnaireMatriculesFromValue(value));
        }
    }
...
function extractOmMissionnaireRealisationRanges(row) {
    ...
    const visit = (value, path) => {
        ...
        const hasLocalMatricule = entries.some(([key, child]) => {
            const fullPath = path ? `${path}/${key}` : key;
            return isKoboMatriculeKey(fullPath) && extractMissionnaireMatriculesFromValue(child).some(Boolean);
        });
...
function extractOmMissionnaireAssignments(row) {
    ...
    const visit = (value, path) => {
        ...
        entries.forEach(([key, child]) => {
            const fullPath = path ? `${path}/${key}` : key;
            if (isKoboMatriculeKey(fullPath)) localMats.push(...extractMissionnaireMatriculesFromValue(child));
        });
...
function getDuplicateDirectFieldValue(row, fieldType, level) {
    const normalizedLevel = normalizeKoboSearchKey(level || '');
    const entries = Object.entries(row || {});
    const isPrimitive = value => value !== null && value !== undefined && typeof value !== 'object';

    for (const [rawKey, value] of entries) {
        if (!isPrimitive(value)) continue;
        const leaf = normalizeKoboSearchKey(String(rawKey).split('/').pop());
        const full = normalizeKoboSearchKey(rawKey);
        const key = `${leaf}_${full}`;
        const hasLevel = !normalizedLevel || key.includes(normalizedLevel);
        if (!hasLevel) continue;

        let matches = false;
        if (fieldType === 'entity') {
            matches = leaf === normalizedLevel;
        } else if (fieldType === 'activite') {
            matches = key.includes('activite') && !key.includes('sous_activite') && !key.includes('sousactivite') && !key.includes('produit');
        } else if (fieldType === 'produit') {
            matches = key.includes('produit') && !key.includes('sous_produit') && !key.includes('sousproduit');
        } else if (fieldType === 'sousActivite') {
            matches = key.includes('sous_activite') || key.includes('sousactivite') || (key.includes('sous') && key.includes('activite'));
        } else if (fieldType === 'declaredLevel') {
            matches = leaf === 'entite' || leaf === 'niveau_entite' || leaf === 'niveau';
        }
        if (matches && cleanSpaces(value) !== '') return cleanSpaces(value);
    }
    return '';
}

function getDuplicateBaseFields(row) {
    // Lecture directe prioritaire : elle évite que « Anomalie Colonne Sous
    // activité finale » soit confondue avec le véritable champ Activité dans
    // les exports JSON/HTML du tableau de bord.
    const dren = getDuplicateDirectFieldValue(row, 'entity', 'dren') ||
        cleanSpaces(getKoboValue(row, ['dren'], ['activite', 'produit', 'budget', 'cisco', 'zap', 'sous'], []));
    const cisco = getDuplicateDirectFieldValue(row, 'entity', 'cisco') ||
        cleanSpaces(getKoboValue(row, ['cisco'], ['activite', 'produit', 'budget', 'dren', 'zap', 'sous'], []));
    const zap = getDuplicateDirectFieldValue(row, 'entity', 'zap') ||
        cleanSpaces(getKoboValue(row, ['zap'], ['activite', 'produit', 'budget', 'dren', 'cisco', 'sous'], []));

    const declared = normalizeKoboSearchKey(getDuplicateDirectFieldValue(row, 'declaredLevel', ''));
    const level = declared.includes('zap') ? 'zap' :
        (declared.includes('cisco') ? 'cisco' :
        (declared.includes('dren') ? 'dren' : (zap ? 'zap' : (cisco ? 'cisco' : 'dren'))));

    const levelLabel = level.toUpperCase();
    const activityKey = level === 'zap' ? 'activiteZap' : (level === 'cisco' ? 'activiteCisco' : 'activiteDren');
    const productKey = level === 'zap' ? 'produitZap' : (level === 'cisco' ? 'produitCisco' : 'produitDren');
    const subActivityKey = level === 'zap' ? 'sousActiviteZap' : (level === 'cisco' ? 'sousActiviteCisco' : 'sousActiviteDren');
    const activityCol = baseColsInfo.find(col => col.key === activityKey);
    const productCol = baseColsInfo.find(col => col.key === productKey);
    const subActivityCol = baseColsInfo.find(col => col.key === subActivityKey);

    const activite = getDuplicateDirectFieldValue(row, 'activite', level) ||
        (activityCol ? cleanSpaces(getKoboValue(row, activityCol.matches, activityCol.ex, activityCol.mustMatch)) : '');
    const produit = getDuplicateDirectFieldValue(row, 'produit', level) ||
        (productCol ? cleanSpaces(getKoboValue(row, productCol.matches, productCol.ex, productCol.mustMatch)) : '');
    const sousActivite = getDuplicateDirectFieldValue(row, 'sousActivite', level) ||
        (subActivityCol ? cleanSpaces(getKoboValue(row, subActivityCol.matches, subActivityCol.ex, subActivityCol.mustMatch)) : '');

    return { dren, cisco, zap, activite, produit, sousActivite, level: levelLabel };
}
...
function buildMissionnaireDuplicateAnalysis(data) {
    ...
    (Array.isArray(data) ? data : []).forEach((row, rowIndex) => {
        ...
        assignments.forEach((assignment, assignmentIndex) => {
            ...
            // Une soumission DREN n'a normalement ni CISCO ni ZAP ; une
            // soumission CISCO peut ne pas avoir de ZAP. Ces champs vides sont
            // des valeurs « non applicables », et non une signature incomplète.
            // Ils restent dans la signature afin que deux niveaux différents ne
            // puissent jamais être confondus.
            const hasAdministrativeEntity = [base.dren, base.cisco, base.zap].some(value => cleanSpaces(value) !== '');
            const signatureComparable = hasAdministrativeEntity &&
                [base.activite, base.produit, base.sousActivite, assignment.matricule]
                    .every(value => cleanSpaces(value) !== '');
            const record = {
    ...
    Object.values(bySignature).forEach(group => {
        const eligible = group.filter(record => record.signatureComparable && record.matricule && record.start && record.end && !record.invalidDateOrder);
...
function extractKoboRecordsFromPayload(payload) {
    ...
    for (const key of ['results', 'resultats', 'data', 'records', 'rows', 'base_kobo', 'flattened_results']) {
...
function normalizeRealisationFieldName(value) {
    return String(value || '')
        .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '_')
...
function parseStrictRealisationNumber(value) {
    ...
    // Une date ISO ne doit jamais devenir la valeur 2026 par l'effet de parseFloat().
    if (/^\d{4}-\d{1,2}-\d{1,2}(?:[T\s].*)?$/.test(raw)) return null;

    const normalized = raw
        .replace(/[\u00A0\u202F\s]/g, '')
        .replace(',', '.');

    // La chaîne entière doit être numérique. On refuse « 2026-07-20 », « 12 unités », etc.
...
function getRealisationFieldPriority(key) {
    const leaf = normalizeRealisationFieldName(String(key || '').split('/').pop());
    const full = normalizeRealisationFieldName(key);

    // Exclusion prioritaire de tous les champs de dates, même si leur nom contient « réalisation ».
    if (/(^|_)(date|debut|fin|start|end|today|jour|mois|annee|submission|suivi_date)(_|$)/.test(full)) return -1;

    if (/^realisations?$/.test(leaf)) return 100;
    if (/^(valeur|volume|nombre|quantite|effectif|montant|taux)_de?_?realisations?$/.test(leaf)) return 95;
    if (/(valeur|volume|nombre|quantite|effectif|montant|taux).*realisation|realisation.*(valeur|volume|nombre|quantite|effectif|montant|taux)/.test(leaf)) return 90;
    if (/(quantite|effectif|montant)_realise(e|es|s)?$/.test(leaf)) return 85;

    return -1;
}

function findRealisationNumericField(row) {
    const candidates = [];
    Object.entries(row || {}).forEach(([key, value], order) => {
        const priority = getRealisationFieldPriority(key);
        if (priority < 0) return;
        const number = parseStrictRealisationNumber(value);
        if (number === null) return;
        candidates.push({ key, number, priority, order });
    });
...
function getRealisationsData() {
    ...
    allData.forEach(row => {
        const dateSubRaw = row['_submission_time'] ? String(row['_submission_time']).substring(0, 10) : null;
        ...
        const labelAct = (sAct && sAct !== 'Non renseigné') ? sAct : (act || 'Non spécifiée');
        const isAnomaly = (!sAct || sAct.toLowerCase() === 'non renseigné');

        // Une seule valeur de réalisation est retenue par soumission.
        // Les dates « DATE_DEBUT_REALISATION » et « DATE_FIN_REALISATION » sont explicitement exclues.
        const realizationField = findRealisationNumericField(row);
        if (!realizationField) return;

        const niveau = vZap ? 'ZAP' : (vCisco ? 'CISCO' : (vDren ? 'DREN' : 'National'));
        const entite = vZap || vCisco || vDren || 'Inconnue';
        ...
        realData.push({
            ...
            entite,
            activite: labelAct,
            valeur: realizationField.number,
            champSource: realizationField.key,
            isAnomaly
        });
    });
