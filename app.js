...
function updateRealisationZapDimensionUi() {
    const config = getRealisationDimensionConfig(getRealisationSelectedDimension('ZAP'));
    const caption = document.getElementById('real-zap-axis-caption');
    if (caption) caption.textContent = `Période début-fin × valeur de réalisation × code ZAP : nom ZAP - ${config.label}`;
...
function renderRealisationDetailTable(level, records, settings, forcedEntities = null) {
    ...
    if (!displayed.length) {
        tbody.innerHTML = '<tr><td colspan="13" class="text-center text-muted py-3">Aucune réalisation à afficher dans le tableau.</td></tr>';
