...
/* ================================================================
   SCROLLBARS HORIZONTALES ET VERTICALES POUR TOUS LES GRAPHIQUES
   - Les graphiques déjà placés dans un conteneur défilable conservent
     leur structure et reçoivent le style universel.
   - Les autres canvas sont automatiquement enveloppés dans une zone
     défilable avec une scène intérieure plus grande.
   - Un MutationObserver applique aussi la règle aux graphiques créés
     dynamiquement (affichage individuel, analyse sémantique, etc.).
   ================================================================ */
...
function resizeChartAfterUniversalScroll(canvas) {
    window.requestAnimationFrame(() => {
        try {
            ...
            if (!chart && typeof Chart !== 'undefined' && Chart.instances) {
                ...
                chart = instances.find(instance => instance && instance.canvas === canvas) || null;
            }
            if (chart && typeof chart.resize === 'function') chart.resize();
        } catch (error) {
            console.warn('Redimensionnement du graphique après ajout des scrollbars impossible :', error);
        }
    });
}
...
function enhanceChartCanvasWithUniversalScrollbars(canvas) {
    if (!(canvas instanceof HTMLCanvasElement) || canvas.dataset.universalScrollbars === '1') return;
    ...
    if (host) {
        ...
        stage = canvas.closest('.timeline-chart-stage, .realisation-chart-stage, .realisation-gantt-individual-stage, .universal-chart-stage, .universal-chart-existing-stage');
        ...
        host.setAttribute('aria-label', ('Zone défilable du graphique ' + (canvas.id || '')).trim());
    ...
    resizeChartAfterUniversalScroll(canvas);
}
...
function resizeAllChartsAfterTabChange() {
    setTimeout(() => {
        ...
        try {
            ...
            console.warn('Redimensionnement général des graphiques impossible :', error);
        }
    }, 100);
}
