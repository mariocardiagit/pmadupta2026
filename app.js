...
function configureAdaptiveChartLegend(chart, canvas, updateChart = true) {
    ...
    profile.datasets.forEach((dataset, index) => {
        if (!String(dataset.label || '').trim() && profile.datasetCount > 1) {
    ...
    if (updateChart && typeof chart.update === 'function') {
        try { chart.update('none'); } catch (error) {
            console.warn('Actualisation de la légende impossible :', error);
        }
    }
    return true;
}
...
function registerAdaptiveChartUiPlugin() {
    ...
    Chart.register({
        ...
        beforeInit(chart) {
            configureAdaptiveChartLegend(chart, chart.canvas, false);
        },
        afterInit(chart) {
        ...
        afterUpdate(chart) {
            scheduleAdaptiveChartLayout(chart.canvas, 10);
        }
    });
