...
function pmaFixedExportAdvancedMeta(chartKey) {
    ...
    const canvasId = `adv-${method.toLowerCase().replace('-', '')}-${level.toLowerCase()}-chart`;
    return { chartKey: normalized, method, level, canvasId, title: `${method} — ${level}` };
}
...
window.exportAdvancedSingleWord = async function(chartKey) {
    try {
        ...
        const blob = await pmaFixedExportCreateDocx(`Rapport ${meta.title}`, [{ ...image, title: meta.title }]);
