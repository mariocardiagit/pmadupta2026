...
const SUBMISSION_TIMELINE_GRANULARITIES = [
    { value: 'day', label: 'Jour', axis: 'Jour de soumission', unit: 'jour' },
    { value: 'week', label: 'Semaine', axis: 'Semaine de soumission', unit: 'semaine' },
    { value: 'month', label: 'Mois', axis: 'Mois de soumission', unit: 'mois' },
    { value: 'quarter', label: 'Trimestre', axis: 'Trimestre de soumission', unit: 'trimestre' },
    { value: 'semester', label: 'Semestre', axis: 'Semestre de soumission', unit: 'semestre' },
    { value: 'year', label: 'Année', axis: 'Année de soumission', unit: 'année' }
];

function getSubmissionTimelineGranularityMeta(granularity) {
    return SUBMISSION_TIMELINE_GRANULARITIES.find(item => item.value === granularity) || SUBMISSION_TIMELINE_GRANULARITIES[0];
}

function getSubmissionTimelineBucket(date, granularity) {
    let bucketDate = new Date(date.getTime());
    let year = bucketDate.getUTCFullYear();
    let month = bucketDate.getUTCMonth();

    if (granularity === 'year') {
        bucketDate = new Date(Date.UTC(year, 0, 1));
    } else if (granularity === 'semester') {
        bucketDate = new Date(Date.UTC(year, Math.floor(month / 6) * 6, 1));
    } else if (granularity === 'quarter') {
        bucketDate = new Date(Date.UTC(year, Math.floor(month / 3) * 3, 1));
    } else if (granularity === 'month') {
        bucketDate = new Date(Date.UTC(year, month, 1));
...
function getNextSubmissionTimelineBucketDate(date, granularity) {
    let next = new Date(date.getTime());
    if (granularity === 'year') next.setUTCFullYear(next.getUTCFullYear() + 1, 0, 1);
    else if (granularity === 'semester') next.setUTCMonth(next.getUTCMonth() + 6, 1);
    else if (granularity === 'quarter') next.setUTCMonth(next.getUTCMonth() + 3, 1);
    else if (granularity === 'month') next.setUTCMonth(next.getUTCMonth() + 1, 1);
    else if (granularity === 'week') next.setUTCDate(next.getUTCDate() + 7);
    else next.setUTCDate(next.getUTCDate() + 1);
    return next;
}

function buildSubmissionTimelineBucketKeys(datedRows, granularity, requestedStartDate, requestedEndDate) {
    if (!datedRows.length) return [];

    let actualDates = datedRows.map(item => item.date).sort((a, b) => a - b);
    let rangeStart = requestedStartDate || actualDates[0];
    let rangeEnd = requestedEndDate || actualDates[actualDates.length - 1];
    let firstBucket = parseSubmissionDate(getSubmissionTimelineBucket(rangeStart, granularity));
    let lastBucket = parseSubmissionDate(getSubmissionTimelineBucket(rangeEnd, granularity));
    if (!firstBucket || !lastBucket) return [];

    let keys = [];
    let cursor = firstBucket;
    let guard = 0;
    while (cursor <= lastBucket && guard < 10000) {
        keys.push(formatISODateUTC(cursor));
        cursor = getNextSubmissionTimelineBucketDate(cursor, granularity);
        guard++;
    }
    return keys;
}
...
function formatSubmissionTimelineLabel(bucketKey, granularity) {
    ...
    if (granularity === 'semester') return `S${Math.floor(month / 6) + 1} ${year}`;
...
function getSubmissionTimelineModeMeta(displayMode) {
    return displayMode === 'cumulative'
        ? {
            label: 'Données cumulées',
            shortLabel: 'Cumulé',
            yAxis: 'Nombre cumulé de soumissions',
            explanation: 'Chaque point additionne toutes les soumissions depuis le début de la période filtrée jusqu’à la période affichée.'
        }
        : {
            label: 'Données détaillées',
            shortLabel: 'Détaillé',
            yAxis: 'Nombre de soumissions par période',
            explanation: 'Chaque point montre uniquement le nombre de soumissions enregistré pendant la période correspondante.'
        };
}

function updateSubmissionTimelineControlState(granularity, displayMode) {
    let meta = getSubmissionTimelineGranularityMeta(granularity);
    let modeMeta = getSubmissionTimelineModeMeta(displayMode);
    let index = SUBMISSION_TIMELINE_GRANULARITIES.findIndex(item => item.value === granularity);

    $('#timeline-zoom-in-btn').prop('disabled', index <= 0);
    $('#timeline-zoom-out-btn').prop('disabled', index >= SUBMISSION_TIMELINE_GRANULARITIES.length - 1);
    $('#timeline-selected-period').text(meta.label);
    $('#timeline-selected-mode').text(modeMeta.label);
    $('#timeline-selection-explanation').text(
        displayMode === 'cumulative'
            ? `${modeMeta.explanation} Le cumul est recalculé selon l’unité « ${meta.label} » choisie.`
            : `Chaque point montre le nombre de soumissions enregistré pendant la période « ${meta.label} » correspondante.`
    );
    $('.timeline-chart-period-badge').text(`${meta.label} · ${modeMeta.shortLabel}`);
}

function changeSubmissionTimelineZoom(direction) {
    let current = $('#timeline-granularity').val() || 'day';
    let index = SUBMISSION_TIMELINE_GRANULARITIES.findIndex(item => item.value === current);
    if (index < 0) index = 0;

    // Zoom + = période plus fine ; Zoom - = période plus large.
    let nextIndex = direction === 'in' ? Math.max(0, index - 1) : Math.min(SUBMISSION_TIMELINE_GRANULARITIES.length - 1, index + 1);
    if (nextIndex === index) return;
    $('#timeline-granularity').val(SUBMISSION_TIMELINE_GRANULARITIES[nextIndex].value);
    renderSubmissionTimelineCharts();
}

function renderOneSubmissionTimelineChart(level, datedRows, bucketKeys, granularity, topLimit, displayMode) {
    ...
    const dashPatterns = [[], [7, 4], [2, 3], [10, 3, 2, 3], [12, 5], [4, 4]];
    const pointStyles = ['circle', 'rectRot', 'triangle', 'rect', 'star', 'crossRot'];
    let datasets = rankedEntities.map(([entity], index) => {
        let color = getSubmissionTimelineColor(index, 1);
        let periodValues = bucketKeys.map(bucket => valuesByEntity[entity][bucket] || 0);
        let chartValues = periodValues;
        if (displayMode === 'cumulative') {
            let runningTotal = 0;
            chartValues = periodValues.map(value => {
                runningTotal += value;
                return runningTotal;
            });
        }
        return {
            ...
            pointBorderWidth: 1,
            pointRadius: bucketKeys.length > 60 ? 0 : 3,
            pointHoverRadius: 6,
    ...
    let granularityMeta = getSubmissionTimelineGranularityMeta(granularity);
    let modeMeta = getSubmissionTimelineModeMeta(displayMode);
    submissionTimelineChartsRefs[level] = new Chart(canvas.getContext('2d'), {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: `Période : ${granularityMeta.label} · ${modeMeta.label}`,
                    color: '#34495e',
                ...
                legend: {
                    position: 'bottom',
                    labels: { usePointStyle: true, boxWidth: 20, padding: 14 }
                },
                tooltip: {
                    callbacks: {
                        title: items => items.length ? `${granularityMeta.label} : ${items[0].label}` : '',
                        label: context => {
                            let value = context.parsed.y;
                            let suffix = displayMode === 'cumulative' ? ' soumission(s) cumulée(s)' : ' soumission(s) pendant cette période';
            ...
            scales: {
                x: {
                    title: { display: true, text: granularityMeta.axis, font: { weight: 'bold' } },
                    ticks: { autoSkip: true, maxTicksLimit: 14, maxRotation: 0, minRotation: 0 },
                    grid: { display: false }
                },
                y: {
                    beginAtZero: true,
                    title: { display: true, text: modeMeta.yAxis, font: { weight: 'bold' } },
                    ticks: { precision: 0 }
                }
            }
        }
...
function renderSubmissionTimelineCharts(data) {
    ...
    let displayMode = $('#timeline-display-mode').val() || 'detailed';
    ...
    if (startDate && endDate && startDate > endDate) {
        ...
        startDate = endDate;
        endDate = swap;
        $('#timeline-date-start').val(formatISODateUTC(startDate));
        $('#timeline-date-end').val(formatISODateUTC(endDate));
    }
    ...
    let bucketKeys = buildSubmissionTimelineBucketKeys(datedRows, granularity, startDate, endDate);
    ...
    updateSubmissionTimelineControlState(granularity, displayMode);
    ...
    if (datedRows.length > 0) {
        ...
        let granularityMeta = getSubmissionTimelineGranularityMeta(granularity);
        $('#timeline-period-label').text(`${first === last ? first : `${first} – ${last}`} · ${granularityMeta.label}`);
    } else {
        $('#timeline-period-label').text(`Aucune date · ${getSubmissionTimelineGranularityMeta(granularity).label}`);
    }

    ['DREN', 'CISCO', 'ZAP'].forEach(level => renderOneSubmissionTimelineChart(level, datedRows, bucketKeys, granularity, topLimit, displayMode));
}
...
function renderAnalysis(data) {
    ...
    // Synchronise les quatre sous-onglets complémentaires placés à la fin de l'onglet 2.
    if (typeof window.refreshAdvancedAnalysisFromMainData === 'function') {
        window.refreshAdvancedAnalysisFromMainData(data);
    }
}
...
$(document).ready(function() {
    ...
    $('#timeline-zoom-in-btn').on('click', function() { changeSubmissionTimelineZoom('in'); });
    $('#timeline-zoom-out-btn').on('click', function() { changeSubmissionTimelineZoom('out'); });
    $('#timeline-granularity, #timeline-display-mode, #timeline-top-entities, #timeline-date-start, #timeline-date-end').on('change', function() { renderSubmissionTimelineCharts(); });
