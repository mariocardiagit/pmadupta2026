...
function getRealisationSelectedDimension(level) {
    ...
    if (levelKey === 'DREN' || levelKey === 'CISCO') {
...
function getRealisationAxisLabel(record, level) {
    const config = getRealisationDimensionConfig(getRealisationSelectedDimension(level));
    return `${record?.entite || 'Entité inconnue'} - ${getRealisationDimensionValue(record, level)}`;
}
...
document.addEventListener('DOMContentLoaded', function(){ try { initRealisationDrenDimensionTabs(); initRealisationCiscoDimensionTabs(); } catch(e) { console.warn(e); } });
