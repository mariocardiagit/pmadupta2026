...
// ============================================================================
// V7.11 — TABLEAUX COMPLETS SOUS LES GRAPHIQUES DE L'ONGLET ANALYSE
// Le tableau est construit à partir du même objet `data` que Chart.js afin de
// garantir une correspondance exacte entre le graphique visible et les valeurs.
// ============================================================================
function submissionTimelineTableNumber(value) {
    const n = Number(value);
    return Number.isFinite(n) ? n.toLocaleString('fr-FR') : '—';
}

function submissionTimelineTableFirstColumn(settings, profile, individual) {
    if (profile.histogram) return 'Tranche de volume';
    if (!individual && (profile.circular || profile.horizontal)) return 'Entité';
    return SUBMISSION_TIMELINE_GRANULARITY_LABELS[settings.granularity] || 'Période';
}

function buildSubmissionTimelineDataTableHtml(title, chartData, settings, profile, options = {}) {
    const labels = Array.isArray(chartData?.labels) ? chartData.labels : [];
    const datasets = Array.isArray(chartData?.datasets) ? chartData.datasets : [];
    const individual = !!options.individual;
    const firstColumn = submissionTimelineTableFirstColumn(settings, profile, individual);
    const safeTitle = escapeSubmissionTimelineHtml(title || 'Données du graphique');
    if (!labels.length || !datasets.length) {
        return `<div class="timeline-data-table-empty">Aucune donnée à afficher dans le tableau correspondant au graphique.</div>`;
    }

    const headerCells = datasets.map(ds => `<th scope="col">${escapeSubmissionTimelineHtml(ds.label || 'Valeur')}</th>`).join('');
    const bodyRows = labels.map((label, rowIndex) => {
        const cells = datasets.map(ds => {
            const raw = Array.isArray(ds.data) ? ds.data[rowIndex] : undefined;
            const value = (raw && typeof raw === 'object' && !Array.isArray(raw))
                ? (raw.y ?? raw.x ?? raw.r ?? '')
                : raw;
            return `<td class="timeline-data-number">${submissionTimelineTableNumber(value)}</td>`;
        }).join('');
        return `<tr><th scope="row">${escapeSubmissionTimelineHtml(String(label ?? ''))}</th>${cells}</tr>`;
    }).join('');

    let footerLabel = settings.mode === 'cumulative' && !profile.histogram ? 'VALEUR FINALE' : 'TOTAL';
    if (profile.histogram) footerLabel = individual ? 'TOTAL DES PÉRIODES' : 'TOTAL DES ENTITÉS';
    const footerCells = datasets.map(ds => {
        const nums = (Array.isArray(ds.data) ? ds.data : []).map(v => {
            if (v && typeof v === 'object' && !Array.isArray(v)) return Number(v.y ?? v.x ?? v.r);
            return Number(v);
        }).filter(Number.isFinite);
        const value = settings.mode === 'cumulative' && !profile.histogram && nums.length ? nums[nums.length - 1] : nums.reduce((sum, v) => sum + v, 0);
        return `<td class="timeline-data-number timeline-data-total-value">${submissionTimelineTableNumber(value)}</td>`;
    }).join('');

    const seriesLabel = datasets.length > 1 ? `${datasets.length} séries` : '1 série';
    return `<section class="timeline-data-table-panel-inner">
        <div class="timeline-data-table-heading">
            <div>
                <strong><i class="fas fa-table"></i> ${safeTitle}</strong>
                <small>Toutes les valeurs du graphique affiché juste au-dessus.</small>
            </div>
            <span>${labels.length.toLocaleString('fr-FR')} ligne(s) · ${seriesLabel}</span>
        </div>
        <div class="timeline-data-table-wrap" tabindex="0" aria-label="Tableau complet des données du graphique">
            <table class="timeline-data-table">
                <thead><tr><th scope="col">${escapeSubmissionTimelineHtml(firstColumn)}</th>${headerCells}</tr></thead>
                <tbody>${bodyRows}</tbody>
                <tfoot><tr><th scope="row">${footerLabel}</th>${footerCells}</tr></tfoot>
            </table>
        </div>
    </section>`;
}

function renderGroupedSubmissionTimelineDataTable(level, chartData, settings, profile) {
    ...
    let host = document.getElementById(`timeline-data-table-${lower}`);
    if (!host) {
        host = document.createElement('div');
        host.id = `timeline-data-table-${lower}`;
        host.className = 'timeline-data-table-panel';
        if (emptyState && emptyState.parentNode === groupedView) groupedView.insertBefore(host, emptyState);
        else groupedView.appendChild(host);
    }
    host.innerHTML = buildSubmissionTimelineDataTableHtml(`TABLEAU COMPLET — Soumissions par ${level}`, chartData, settings, profile, { level, individual: false });
    host.style.display = '';
}

function clearGroupedSubmissionTimelineDataTable(level) {
    const host = document.getElementById(`timeline-data-table-${String(level).toLowerCase()}`);
...
function renderGroupedSubmissionTimelineChart(level, datedRows, bucketKeys, settings) {
    ...
    if (!levelData.series.length || (!bucketKeys.length && !profile.totals)) {
        ...
        clearGroupedSubmissionTimelineDataTable(level);
        return;
    }
    canvas.style.display = 'block';
    emptyState.style.display = 'none';
    ...
    renderGroupedSubmissionTimelineDataTable(level, data, settings, profile);
}
...
function renderIndividualSubmissionTimelineLevel(level, datedRows, bucketKeys, settings) {
    ...
    pageItems.forEach((item, pageIndex) => {
        ...
        col.innerHTML = `<article class="timeline-individual-card h-100">
            ...
            <div class="timeline-individual-data-table"></div>
        </article>`;
        grid.appendChild(col);
        const canvas = col.querySelector('canvas');
        const data = buildIndividualSubmissionChartData(item, labels, settings, profile);
        const tableHost = col.querySelector('.timeline-individual-data-table');
        if (tableHost) tableHost.innerHTML = buildSubmissionTimelineDataTableHtml(`TABLEAU COMPLET — ${item.entity}`, data, settings, profile, { level, individual: true });
