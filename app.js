...
function getRealisationSelectedDimension(level) {
    ...
    if (levelKey === 'DREN') {
        const active = document.querySelector('[data-real-level-dimension="DREN"].active');
...
function getRealisationYAxisTitle(level, individual = false) {
    ...
    if (levelKey === 'DREN') {
        ...
        return individual ? `${config.label} des missions DREN` : `Code DREN : nom DREN - ${config.label}`;
    }
...
document.addEventListener('DOMContentLoaded', function(){ try { initRealisationDrenDimensionTabs(); } catch(e) { console.warn(e); } });
