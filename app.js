...
function createDrenAggregatedDurationChart(canvas,stage,aggregates){const rows=[...aggregates].sort((a,b)=>String(a.entite).localeCompare(String(b.entite),'fr')||getDrenAggregateLabel(a).localeCompare(getDrenAggregateLabel(b),'fr')),maxDays=Math.max(1,...rows.map(r=>Number(r.totalDays||0))),w=Math.max(1180,760+Math.min(80,rows.length)*26),h=Math.max(470,190+rows.length*72);stage.classList.add('realisation-gantt-stage');stage.style.width=`${w}px`;stage.style.minWidth=`${w}px`;stage.style.height=`${h}px`;const ds={label:'Jours de réalisation',data:rows.map(r=>r.totalDays),_aggregates:rows,backgroundColor:'rgba(25, 135, 84, .72)',borderColor:'rgba(25, 135, 84, 1)',borderWidth:1.5,borderRadius:5,borderSkipped:false,barPercentage:.52,categoryPercentage:.7},d=getRealisationSelectedDimension('DREN'),cfg=getRealisationDimensionConfig(d),chart=new Chart(canvas.getContext('2d'),{type:'bar',data:{labels:rows.map(getDrenAggregateLabel),datasets:[ds]},plugins:[drenAggregationLabelsPlugin],options:{responsive:true,maintainAspectRatio:false,indexAxis:'y',layout:{padding:{right:230,top:20,bottom:25,left:10}},plugins:{legend:{display:false},tooltip:{callbacks:{title:items=>rows[items?.[0]?.dataIndex]?.entite||'DREN',label:ctx=>{const r=rows[ctx.dataIndex];return [`${cfg.label} : ${d==='activite'?r.activiteBase:r.produit}`,`Réalisation totale : ${Number(r.valeur||0).toLocaleString('fr-FR')}`,`Jours de réalisation : ${Number(r.totalDays||0)}`,getDrenAggregateFormulaText(r),'Détails complets : bouton « Afficher les Détails »'];}}}},scales:{x:{beginAtZero:true,suggestedMax:Math.ceil(maxDays*1.2),title:{display:true,text:'Nombre total de jours de réalisation (somme des durées inclusives)',font:{weight:'bold'}},ticks:{precision:0}},y:{title:{display:true,text:`Code DREN : nom DREN - ${cfg.label}`,font:{weight:'bold'}},ticks:{autoSkip:false,font:{size:10},callback:function(v){return wrapRealisationGanttAxisLabel(this.getLabelForValue(v),false);}},grid:{display:false}}}}});realisationTimelineChartsRefs.DREN.push({chart,name:`dren_${d}_agregation_hierarchique`});return chart;}
function renderDrenAggregatedDetailTable(rows,d){const section=document.getElementById('real-detail-section-dren'),tbody=document.getElementById('real-detail-table-dren'),count=document.getElementById('real-detail-count-dren'),total=document.getElementById('real-detail-total-dren'),cfg=getRealisationDimensionConfig(d),h=document.getElementById('real-detail-label-heading-dren'),sh=document.getElementById('real-detail-start-heading-dren'),eh=document.getElementById('real-detail-end-heading-dren');if(!section||!tbody)return;section.classList.toggle('d-none',!rows.length);if(count)count.textContent=rows.length.toLocaleString('fr-FR');if(total)total.textContent=rows.reduce((s,r)=>s+Number(r.valeur||0),0).toLocaleString('fr-FR');if(h)h.textContent=cfg.label;if(sh)sh.textContent='Type de calcul';if(eh)eh.textContent='Composition';tbody.innerHTML=rows.map((r,i)=>{const comp=d==='produit'?`${r.components?.length||0} Sous Activité(s)`:`${r.products?.length||0} Produit(s)`,label=d==='activite'?r.activiteBase:r.produit;return `<tr><td class="text-center fw-bold">${i+1}</td><td class="text-center"><span class="badge bg-secondary">DREN</span></td><td class="fw-semibold">${escapeRealisationHtml(r.entite)}</td><td>${escapeRealisationHtml(label)}</td><td class="text-center">Agrégation hiérarchique</td><td class="text-center">${escapeRealisationHtml(comp)}</td><td class="text-end fw-bold">${Number(r.totalDays||0)} jour(s)</td><td class="text-end fw-bold text-success">${Number(r.valeur||0).toLocaleString('fr-FR')}</td><td class="text-center">—</td><td class="text-center">—</td><td class="text-center">${r.components?.length||0} source(s)</td><td class="text-center">${r.isAnomaly?'<span class="badge bg-danger">Anomalies de liaison</span>':'<span class="badge bg-success">Aucune</span>'}</td><td class="text-center">${r.isDoublon?'<span class="badge bg-danger">DOUBLON</span>':'<span class="badge bg-success">Mission distincte</span>'}</td><td class="text-center"><button type="button" class="btn btn-success btn-sm fw-bold" onclick="window.openDrenAggregationDetails('${escapeRealisationHtml(r.id)}')"><i class="fas fa-search-plus"></i> Afficher les Détails</button></td></tr>`;}).join('');}
...
function createCiscoAggregatedDurationChart(canvas,stage,aggregates){const rows=[...aggregates].sort((a,b)=>String(a.entite).localeCompare(String(b.entite),'fr')||getCiscoAggregateLabel(a).localeCompare(getCiscoAggregateLabel(b),'fr')),maxDays=Math.max(1,...rows.map(r=>Number(r.totalDays||0))),w=Math.max(1180,760+Math.min(80,rows.length)*26),h=Math.max(470,190+rows.length*72);stage.classList.add('realisation-gantt-stage');stage.style.width=`${w}px`;stage.style.minWidth=`${w}px`;stage.style.height=`${h}px`;const ds={label:'Jours de réalisation',data:rows.map(r=>r.totalDays),_aggregates:rows,backgroundColor:'rgba(13, 110, 253, .72)',borderColor:'rgba(13, 110, 253, 1)',borderWidth:1.5,borderRadius:5,borderSkipped:false,barPercentage:.52,categoryPercentage:.7},d=getRealisationSelectedDimension('CISCO'),cfg=getRealisationDimensionConfig(d),chart=new Chart(canvas.getContext('2d'),{type:'bar',data:{labels:rows.map(getCiscoAggregateLabel),datasets:[ds]},plugins:[ciscoAggregationLabelsPlugin],options:{responsive:true,maintainAspectRatio:false,indexAxis:'y',layout:{padding:{right:230,top:20,bottom:25,left:10}},plugins:{legend:{display:false},tooltip:{callbacks:{title:items=>rows[items?.[0]?.dataIndex]?.entite||'CISCO',label:ctx=>{const r=rows[ctx.dataIndex];return [`${cfg.label} : ${d==='activite'?r.activiteBase:r.produit}`,`Réalisation totale : ${Number(r.valeur||0).toLocaleString('fr-FR')}`,`Jours de réalisation : ${Number(r.totalDays||0)}`,getCiscoAggregateFormulaText(r),'Détails complets : bouton « Afficher les Détails »'];}}}},scales:{x:{beginAtZero:true,suggestedMax:Math.ceil(maxDays*1.2),title:{display:true,text:'Nombre total de jours de réalisation (somme des durées inclusives)',font:{weight:'bold'}},ticks:{precision:0}},y:{title:{display:true,text:`Code CISCO : nom CISCO - ${cfg.label}`,font:{weight:'bold'}},ticks:{autoSkip:false,font:{size:10},callback:function(v){return wrapRealisationGanttAxisLabel(this.getLabelForValue(v),false);}},grid:{display:false}}}}});realisationTimelineChartsRefs.CISCO.push({chart,name:`cisco_${d}_agregation_hierarchique`});return chart;}
function renderCiscoAggregatedDetailTable(rows,d){const section=document.getElementById('real-detail-section-cisco'),tbody=document.getElementById('real-detail-table-cisco'),count=document.getElementById('real-detail-count-cisco'),total=document.getElementById('real-detail-total-cisco'),cfg=getRealisationDimensionConfig(d),h=document.getElementById('real-detail-label-heading-cisco'),sh=document.getElementById('real-detail-start-heading-cisco'),eh=document.getElementById('real-detail-end-heading-cisco');if(!section||!tbody)return;section.classList.toggle('d-none',!rows.length);if(count)count.textContent=rows.length.toLocaleString('fr-FR');if(total)total.textContent=rows.reduce((s,r)=>s+Number(r.valeur||0),0).toLocaleString('fr-FR');if(h)h.textContent=cfg.label;if(sh)sh.textContent='Type de calcul';if(eh)eh.textContent='Composition';tbody.innerHTML=rows.map((r,i)=>{const comp=d==='produit'?`${r.components?.length||0} Sous Activité(s)`:`${r.products?.length||0} Produit(s)`,label=d==='activite'?r.activiteBase:r.produit;return `<tr><td class="text-center fw-bold">${i+1}</td><td class="text-center"><span class="badge bg-secondary">CISCO</span></td><td class="fw-semibold">${escapeRealisationHtml(r.entite)}</td><td>${escapeRealisationHtml(label)}</td><td class="text-center">Agrégation hiérarchique</td><td class="text-center">${escapeRealisationHtml(comp)}</td><td class="text-end fw-bold">${Number(r.totalDays||0)} jour(s)</td><td class="text-end fw-bold text-primary">${Number(r.valeur||0).toLocaleString('fr-FR')}</td><td class="text-center">—</td><td class="text-center">—</td><td class="text-center">${r.components?.length||0} source(s)</td><td class="text-center">${r.isAnomaly?'<span class="badge bg-danger">Anomalies de liaison</span>':'<span class="badge bg-success">Aucune</span>'}</td><td class="text-center">${r.isDoublon?'<span class="badge bg-danger">DOUBLON</span>':'<span class="badge bg-success">Mission distincte</span>'}</td><td class="text-center"><button type="button" class="btn btn-primary btn-sm fw-bold" onclick="window.openCiscoAggregationDetails('${escapeRealisationHtml(r.id)}')"><i class="fas fa-search-plus"></i> Afficher les Détails</button></td></tr>`;}).join('');}
