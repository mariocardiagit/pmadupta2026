...
function updateDashboardDataSourceUi(mode, message = '') {
    ...
    if (status) {
        ...
        status.innerHTML = message || (sourceMode === 'kobo'
            ...
            : '<i class="fas fa-file-code"></i> <strong>Source active :</strong> Fichier JSON. Importez un fichier pour remplacer automatiquement les données des graphiques et tableaux.');
    }
}
...
async function importKoboJsonFile(file) {
    if (!file) return;
    $('#loading-box').show();
    $('#loading-text').text(`Import du fichier JSON : ${file.name}…`);
    $('#error-box').hide();
    try {
        await loadDictionaryAutomatically();
        ...
        updateDashboardDataSourceUi('json', `<i class="fas fa-file-code"></i> <strong>Source active :</strong> ${escapeRealisationHtml(file.name)} — ${rows.length.toLocaleString('fr-FR')} enregistrement(s). Tous les graphiques et tableaux ont été recalculés depuis ce fichier JSON.`);
    } catch (error) {
        ...
        $('#sync-status').html('<span class="badge bg-danger sync-badge">Échec import JSON</span>');
    } finally {
        $('#loading-box').hide();
...
window.reloadSelectedDataSource = async function() {
    ...
    if (mode === 'json') {
        if (Array.isArray(lastImportedJsonRows) && lastImportedJsonRows.length) {
            window.importKoboRecords(lastImportedJsonRows, `JSON : ${lastImportedJsonFilename || 'fichier importé'}`);
            updateDashboardDataSourceUi('json', `<i class="fas fa-file-code"></i> <strong>Source active :</strong> ${escapeRealisationHtml(lastImportedJsonFilename || 'Fichier JSON')} — données rechargées.`);
            return;
        }
...
function setupDashboardDataSourceControls() {
    document.querySelectorAll('input[name="data-source-mode"]').forEach(input => {
        ...
        input.addEventListener('change', async () => {
            const mode = getSelectedDashboardDataSource();
            updateDashboardDataSourceUi(mode);
            if (mode === 'kobo') await fetchData();
            else if (lastImportedJsonRows?.length) {
                window.importKoboRecords(lastImportedJsonRows, `JSON : ${lastImportedJsonFilename || 'fichier importé'}`);
                updateDashboardDataSourceUi('json', `<i class="fas fa-file-code"></i> <strong>Source active :</strong> ${escapeRealisationHtml(lastImportedJsonFilename || 'Fichier JSON')} — ${lastImportedJsonRows.length.toLocaleString('fr-FR')} enregistrement(s).`);
            }
        });
    });
    const jsonInput = document.getElementById('json-file');
