...
function updateRealisationDrenDimensionUi() {
    const config = getRealisationDimensionConfig(getRealisationSelectedDimension('DREN'));
    const caption = document.getElementById('real-dren-axis-caption');
    if (caption) caption.textContent = `Période début-fin × valeur de réalisation × code DREN : nom DREN - ${config.label}`;
...
function updateRealisationCiscoDimensionUi() {
    const config = getRealisationDimensionConfig(getRealisationSelectedDimension('CISCO'));
    const caption = document.getElementById('real-cisco-axis-caption');
    if (caption) caption.textContent = `Période début-fin × valeur de réalisation × code CISCO : nom CISCO - ${config.label}`;
...
function buildZapAggregationDetailHtml(r){if(!r)return'<p>Détail indisponible.</p>';const label=r.aggregationDimension==='activite'?r.activiteBase:r.produit,kind=r.aggregationDimension==='activite'?'Activité':'Produit';let h=`<div class="zap-aggregation-summary-card"><span class="zap-aggregation-badge">${kind}</span><h6 class="mt-2 mb-1">${escapeRealisationHtml(r.entite)} — ${escapeRealisationHtml(label)}</h6><div><strong>Réalisation totale :</strong> ${Number(r.valeur||0).toLocaleString('fr-FR')} &nbsp; | &nbsp; <strong>Jours de réalisation :</strong> ${Number(r.totalDays||0).toLocaleString('fr-FR')} jour(s)</div><div class="zap-aggregation-formula mt-2">${escapeRealisationHtml(getZapAggregateFormulaText(r))}</div></div>`;
if(r.aggregationDimension==='produit'){h+=`<h6>Sous Activités composant ce Produit</h6><div class="table-responsive"><table class="table table-sm table-bordered zap-aggregation-component-table"><thead><tr><th>#</th><th>Sous Activité</th><th>Date début</th><th>Date fin</th><th>Jours</th><th>Réalisation</th><th>Statut</th></tr></thead><tbody>`;(r.components||[]).forEach((x,i)=>{h+=`<tr><td>${i+1}</td><td>${escapeRealisationHtml(x.sousActivite||x.activite||'Non spécifiée')}</td><td>${formatZapComponentDate(x.dateStart)}</td><td>${formatZapComponentDate(x.dateEnd)}</td><td class="text-end">${Number(getRealisationInclusiveDuration(x)||0)}</td><td class="text-end fw-bold">${Number(x.valeur||0).toLocaleString('fr-FR')}</td><td>${escapeRealisationHtml(getRealisationStatusText(x,'Valide'))}</td></tr>`;});h+=`</tbody><tfoot><tr class="fw-bold"><td colspan="4">TOTAL PRODUIT</td><td class="text-end">${Number(r.totalDays||0)}</td><td class="text-end">${Number(r.valeur||0).toLocaleString('fr-FR')}</td><td></td></tr></tfoot></table></div>`;}else{h+=`<h6>Produits composant cette Activité</h6><div class="table-responsive"><table class="table table-sm table-bordered zap-aggregation-component-table"><thead><tr><th>#</th><th>Produit</th><th>Sous Activités</th><th>Jours</th><th>Réalisation Produit</th><th>Formule</th></tr></thead><tbody>`;(r.products||[]).forEach((p,i)=>{h+=`<tr><td>${i+1}</td><td>${escapeRealisationHtml(p.produit)}</td><td class="text-end">${p.components?.length||0}</td><td class="text-end">${Number(p.totalDays||0)}</td><td class="text-end fw-bold">${Number(p.valeur||0).toLocaleString('fr-FR')}</td><td><code>${escapeRealisationHtml(getZapAggregateFormulaText(p))}</code></td></tr>`;});h+=`</tbody><tfoot><tr class="fw-bold"><td colspan="3">TOTAL ACTIVITÉ</td><td class="text-end">${Number(r.totalDays||0)}</td><td class="text-end">${Number(r.valeur||0).toLocaleString('fr-FR')}</td><td></td></tr></tfoot></table></div><div class="alert alert-light border">Chaque Produit est lui-même calculé comme la somme de ses Sous Activités. Les dates élémentaires restent uniquement dans les détails afin de ne pas inventer une période continue.</div>`;}return h;}
window.openZapAggregationDetails=function(id=null){const body=document.getElementById('zapAggregationDetailsModalBody');if(!body)return;const rows=id?[zapAggregationDetailStore.get(id)].filter(Boolean):zapAggregationCurrentIds.map(k=>zapAggregationDetailStore.get(k)).filter(Boolean);body.innerHTML=rows.length?rows.map(buildZapAggregationDetailHtml).join(''):'<p class="text-muted mb-0">Sélectionnez l’onglet Produit ou Activité afin d’afficher la décomposition de la formule.</p>';const el=document.getElementById('zapAggregationDetailsModal');if(el&&window.bootstrap?.Modal)bootstrap.Modal.getOrCreateInstance(el).show();};
window.openZapRawRealisationDetails=function(id,index){const r=zapLastRawDisplayedRecords[Number(index)]||zapLastRawDisplayedRecords.find(x=>String(x.id||'')===String(id||'')),body=document.getElementById('zapAggregationDetailsModalBody');if(!body)return;body.innerHTML=r?`<div class="zap-aggregation-summary-card"><span class="zap-aggregation-badge">Réalisation élémentaire</span><h6 class="mt-2">${escapeRealisationHtml(r.entite)} — ${escapeRealisationHtml(getRealisationDimensionValue(r,'ZAP'))}</h6><div class="row g-2"><div class="col-md-3"><strong>Début :</strong> ${formatZapComponentDate(r.dateStart)}</div><div class="col-md-3"><strong>Fin :</strong> ${formatZapComponentDate(r.dateEnd)}</div><div class="col-md-3"><strong>Durée :</strong> ${Number(getRealisationInclusiveDuration(r)||0)} jour(s)</div><div class="col-md-3"><strong>Réalisation :</strong> ${Number(r.valeur||0).toLocaleString('fr-FR')}</div><div class="col-12"><strong>Statut :</strong> ${escapeRealisationHtml(getRealisationStatusText(r,'Valide'))}</div></div></div>`:'<p>Détail indisponible.</p>';const el=document.getElementById('zapAggregationDetailsModal');if(el&&window.bootstrap?.Modal)bootstrap.Modal.getOrCreateInstance(el).show();};
...
function createZapAggregatedDurationChart(canvas,stage,aggregates){const rows=[...aggregates].sort((a,b)=>String(a.entite).localeCompare(String(b.entite),'fr')||getZapAggregateLabel(a).localeCompare(getZapAggregateLabel(b),'fr')),maxDays=Math.max(1,...rows.map(r=>Number(r.totalDays||0))),w=Math.max(1180,760+Math.min(80,rows.length)*26),h=Math.max(470,190+rows.length*72);stage.classList.add('realisation-gantt-stage');stage.style.width=`${w}px`;stage.style.minWidth=`${w}px`;stage.style.height=`${h}px`;const ds={label:'Jours de réalisation',data:rows.map(r=>r.totalDays),_aggregates:rows,backgroundColor:'rgba(243, 156, 18, .72)',borderColor:'rgba(211, 84, 0, 1)',borderWidth:1.5,borderRadius:5,borderSkipped:false,barPercentage:.52,categoryPercentage:.7},d=getRealisationSelectedDimension('ZAP'),cfg=getRealisationDimensionConfig(d),chart=new Chart(canvas.getContext('2d'),{type:'bar',data:{labels:rows.map(getZapAggregateLabel),datasets:[ds]},plugins:[zapAggregationLabelsPlugin],options:{responsive:true,maintainAspectRatio:false,indexAxis:'y',layout:{padding:{right:230,top:20,bottom:25,left:10}},plugins:{legend:{display:false},tooltip:{callbacks:{title:items=>rows[items?.[0]?.dataIndex]?.entite||'ZAP',label:ctx=>{const r=rows[ctx.dataIndex];return [`${cfg.label} : ${d==='activite'?r.activiteBase:r.produit}`,`Réalisation totale : ${Number(r.valeur||0).toLocaleString('fr-FR')}`,`Jours de réalisation : ${Number(r.totalDays||0)}`,getZapAggregateFormulaText(r),'Détails complets : bouton « Afficher les Détails »'];}}}},scales:{x:{beginAtZero:true,suggestedMax:Math.ceil(maxDays*1.2),title:{display:true,text:'Nombre total de jours de réalisation (somme des durées inclusives)',font:{weight:'bold'}},ticks:{precision:0}},y:{title:{display:true,text:`Code ZAP : nom ZAP - ${cfg.label}`,font:{weight:'bold'}},ticks:{autoSkip:false,font:{size:10},callback:function(v){return wrapRealisationGanttAxisLabel(this.getLabelForValue(v),false);}},grid:{display:false}}}}});realisationTimelineChartsRefs.ZAP.push({chart,name:`zap_${d}_agregation_hierarchique`});return chart;}
function renderZapAggregatedDetailTable(rows,d){const section=document.getElementById('real-detail-section-zap'),tbody=document.getElementById('real-detail-table-zap'),count=document.getElementById('real-detail-count-zap'),total=document.getElementById('real-detail-total-zap'),cfg=getRealisationDimensionConfig(d),h=document.getElementById('real-detail-label-heading-zap'),sh=document.getElementById('real-detail-start-heading-zap'),eh=document.getElementById('real-detail-end-heading-zap');if(!section||!tbody)return;section.classList.toggle('d-none',!rows.length);if(count)count.textContent=rows.length.toLocaleString('fr-FR');if(total)total.textContent=rows.reduce((s,r)=>s+Number(r.valeur||0),0).toLocaleString('fr-FR');if(h)h.textContent=cfg.label;if(sh)sh.textContent='Type de calcul';if(eh)eh.textContent='Composition';tbody.innerHTML=rows.map((r,i)=>{const comp=d==='produit'?`${r.components?.length||0} Sous Activité(s)`:`${r.products?.length||0} Produit(s)`,label=d==='activite'?r.activiteBase:r.produit;return `<tr><td class="text-center fw-bold">${i+1}</td><td class="text-center"><span class="badge bg-secondary">ZAP</span></td><td class="fw-semibold">${escapeRealisationHtml(r.entite)}</td><td>${escapeRealisationHtml(label)}</td><td class="text-center">Agrégation hiérarchique</td><td class="text-center">${escapeRealisationHtml(comp)}</td><td class="text-end fw-bold">${Number(r.totalDays||0)} jour(s)</td><td class="text-end fw-bold text-warning">${Number(r.valeur||0).toLocaleString('fr-FR')}</td><td class="text-center">—</td><td class="text-center">—</td><td class="text-center">${r.components?.length||0} source(s)</td><td class="text-center">${r.isAnomaly?'<span class="badge bg-danger">Anomalies de liaison</span>':'<span class="badge bg-success">Aucune</span>'}</td><td class="text-center">${r.isDoublon?'<span class="badge bg-danger">DOUBLON</span>':'<span class="badge bg-success">Mission distincte</span>'}</td><td class="text-center"><button type="button" class="btn btn-warning btn-sm fw-bold" onclick="window.openZapAggregationDetails('${escapeRealisationHtml(r.id)}')"><i class="fas fa-search-plus"></i> Afficher les Détails</button></td></tr>`;}).join('');}
...
function renderRealisationDetailTable(level, records, settings, forcedEntities = null) {
    ...
    if (!displayed.length) {
        tbody.innerHTML = `<tr><td colspan="${levelKey === 'ZAP' ? 14 : 13}" class="text-center text-muted py-3">Aucune réalisation à afficher dans le tableau.</td></tr>`;
