...
function isVerifiedBackupFormat(format) {
    return format === KOBO_VERIFIED_STANDARD_FORMAT || format === KOBO_VERIFIED_COMPLETE_FORMAT;
}
...
function inspectRecordCoherence(records) {
    ...
    rows.forEach((row, index) => {
        ...
        Object.entries(row).forEach(([key, value]) => {
            ...
            const normalized = normalizeRealisationKey(key);
            if (!/(^|_)(date|submission_time|start|end|debut|fin)(_|$)/.test(normalized)) return;
            const text = String(value).trim();
            if (typeof value !== 'string' && !(value instanceof Date)) return;
            if (/^\d{4}-\d{1,2}-\d{1,2}/.test(text) || /^\d{1,2}[\/.\-]\d{1,2}[\/.\-]\d{4}/.test(text)) return;
            // Ne pas bloquer : certains champs contenant « date » peuvent être des libellés.
            if (text.length <= 40 && /\d/.test(text) && Number.isNaN(Date.parse(text))) suspiciousDates += 1;
        });
    });
    ...
    if (suspiciousDates) warnings.push(`${suspiciousDates} valeur(s) de date potentiellement atypique(s)`);
    return { invalidObjects, duplicateIds, withId, suspiciousDates, warnings };
}
...
async function validateImportedKoboJson(payload, filename = '') {
    ...
    if (coherence.invalidObjects) {
        ...
        setJsonVerificationCheck('coherence', 'warning', coherence.warnings.join(' ; '));
    ...
    if (attachments.length) {
        ...
        if (attachmentErrors) {
            ...
            if (expectedRoot) {
                ...
                } else setJsonVerificationCheck('attachments', 'ok', `${attachments.length} pièce(s) jointe(s) Base64 conformes · SHA-256 global conforme.`);
            } else setJsonVerificationCheck('attachments', 'ok', `${attachments.length} pièce(s) jointe(s) Base64 lisibles et contrôlées.`);
        }
    } else {
        setJsonVerificationCheck('attachments', 'warning', verifiedFormat && format === KOBO_VERIFIED_COMPLETE_FORMAT ? 'Aucune pièce jointe incorporée dans cette sauvegarde complète.' : 'Aucune pièce jointe Base64 à contrôler dans ce fichier.');
    }
...
async function buildVerifiedBackupEnvelope(records, mode = 'standard', extra = {}) {
    ...
    const manifestCore = {
        backup_format: mode === 'complete' ? KOBO_VERIFIED_COMPLETE_FORMAT : KOBO_VERIFIED_STANDARD_FORMAT,
        backup_version: 2,
    ...
    return {
        ...
        records: rows,
        ...(mode === 'complete' ? { attachments, attachment_errors: extra.attachment_errors || [] } : {})
    };
}
...
async function importKoboJsonFile(file) {
    ...
    try {
        ...
        setJsonVerificationVerdict(verification.authenticity === 'verified' ? 'ok' : 'warning', verification.authenticity === 'verified' ? 'CERTIFIÉ' : 'IMPORTABLE', `<strong>VERDICT : FICHIER IMPORTABLE.</strong> ${rows.length.toLocaleString('fr-FR')} enregistrement(s). ${integrityText} ${authText}`);
...
async function exportKoboCompleteBase64Json() {
    ...
    try {
        ...
        const now = new Date();
        const stamp = `${now.getFullYear()}${String(now.getMonth()+1).padStart(2,'0')}${String(now.getDate()).padStart(2,'0')}`;
        const payload = await buildVerifiedBackupEnvelope(allData, 'complete', {
            usage: {
        ...
        if (progress) {
            progress.innerHTML = failures.length
                ...
                : `<strong>Sauvegarde autonome complète générée :</strong> ${embedded.length.toLocaleString('fr-FR')} pièce(s) jointe(s) incorporée(s) en Base64 · SHA-256 des enregistrements : <code>${payload.verification.records_sha256.slice(0,16)}…</code>.`;
        }
    } finally {
