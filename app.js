...
function isOmMissionnaireRealisationDateKey(path, boundary) {
    const key = normalizeKoboSearchKey(path);
    const hasDate = key.includes('date');
    const hasRealisation = key.includes('realis');
    const hasSousActivite = key.includes('sous_activ') || key.includes('sousactiv');
    const hasMissionnaire = key.includes('missionnaire') || key.includes('ordre_mission') || /(^|_)om($|_)/.test(key);
    const hasBoundary = boundary === 'start'
        ? (key.includes('debut') || key.includes('start'))
        : (key.includes('fin') || key.includes('end'));
    return hasDate && hasRealisation && hasBoundary && (hasSousActivite || hasMissionnaire);
}
...
function extractOmMissionnaireRealisationRanges(row) {
    ...
    const visit = (value, path) => {
        ...
        Object.entries(value).forEach(([key, child]) => {
            ...
            if (!date) return;
            if (isOmMissionnaireRealisationDateKey(fullPath, 'start')) localStarts.push(date);
            if (isOmMissionnaireRealisationDateKey(fullPath, 'end')) localEnds.push(date);
        });
...
function extractOmMissionnaireAssignments(row) {
    ...
    const visit = (value, path) => {
        ...
        Object.entries(value).forEach(([key, child]) => {
            const fullPath = path ? `${path}/${key}` : key;
            if (isKoboMatriculeKey(fullPath)) {
                localMats.push(...primitiveValues(child));
                return;
            }
            if (isOmMissionnaireRealisationDateKey(fullPath, 'start')) {
                localStarts.push(...primitiveValues(child));
                return;
            }
            if (isOmMissionnaireRealisationDateKey(fullPath, 'end')) {
    ...
    // Repli pour les réponses Kobo aplaties ou les anciennes structures où le
    // matricule et les dates ne se trouvent pas dans le même objet JavaScript.
    ...
    if (!assignments.some(a => a.matricule) && fallbackMats.length) {
        if (fallbackRanges.length) {
            const count = Math.max(fallbackMats.length, fallbackRanges.length);
            for (let i = 0; i < count; i++) {
                const range = fallbackRanges[i] || fallbackRanges[0] || {};
                assignments.push({ matricule: fallbackMats[i] || fallbackMats[0], start: range.start || null, end: range.end || null, sourcePath: 'fallback' });
            }
        } else {
            fallbackMats.forEach(matricule => assignments.push({ matricule, start: null, end: null, sourcePath: 'fallback' }));
        }
    } else if (assignments.length && fallbackMats.length) {
    ...
    return assignments.filter(item => {
        const key = `${item.matricule}|${item.start ? item.start.getTime() : ''}|${item.end ? item.end.getTime() : ''}|${item.sourcePath}`;
        if (seen.has(key)) return false;
        seen.add(key);
...
function renderTable(data) {
    ...
    data.forEach((row, idx) => {
        ...
        const duplicateIssuesForRow = rowToDuplicateIssues[idx] || [];
        ...
        if (doublonsForRow.length > 0) {
            ...
            doublonHtml = duplicateIssuesForRow.map(issue => `<span class="badge bg-warning text-dark shadow-sm mb-1 text-wrap" style="font-size: 0.76rem; line-height: 1.25; max-width: 230px; white-space: normal;"><i class="fas fa-calendar-exclamation"></i> ${issue}</span>`).join('<br>');
