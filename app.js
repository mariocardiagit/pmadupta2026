...
/* ======================================================================
   ...
   2) AllOrigins ; 3) CodeTabs ; 4) corsproxy.io.
...
// la chaîne robuste Direct KoboToolbox -> AllOrigins -> CodeTabs -> corsproxy.io.
...
fetchData = window.fetchData = async function() {
    ...
    koboSourceFetchPromise = (async () => {
        ...
        if (typeof updateDashboardDataSourceUi === 'function') {
            updateDashboardDataSourceUi('kobo', '<i class="fas fa-cloud-download-alt"></i> <strong>Source active :</strong> KoboToolbox — récupération des enregistrements en cours…');
        }
        ...
        try {
            ...
            // STRATÉGIE CONFIRMÉE FONCTIONNELLE PAR L'UTILISATEUR.
            const fetchUrls = [
                { name: 'Direct KoboToolbox', url: koboUrl },
                { name: 'AllOrigins', url: 'https://api.allorigins.win/raw?url=' + encodeURIComponent(koboUrl) },
                { name: 'CodeTabs', url: 'https://api.codetabs.com/v1/proxy?quest=' + encodeURIComponent(koboUrl) },
                { name: 'corsproxy.io', url: 'https://corsproxy.io/?' + encodeURIComponent(koboUrl) }
            ];
            ...
            // Boucle anti-blocage : on teste chaque méthode dans l'ordre.
            for (const candidate of fetchUrls) {
                try {
                    console.log('Tentative de connexion via :', candidate.name, candidate.url);
                    response = await fetch(candidate.url, { cache: 'no-store' });
                    if (response.ok) {
                        fetchSuccess = true;
                    ...
                    errors.push(`${candidate.name}: ${e?.message || e}`);
                    console.warn('Le navigateur a bloqué l’accès via :', candidate.name, e);
                }
            }

            if (!fetchSuccess || !response) {
                throw new Error(
                    'Toutes les méthodes de connexion KoboToolbox ont échoué. ' +
                    (errors.length ? errors.join(' | ') : 'Aucune réponse exploitable.')
                );
            }
            ...
            allData = allData.filter(row => row !== null && typeof row === 'object' && !Array.isArray(row));

            if (!allData.length) {
                throw new Error('KoboToolbox a répondu correctement, mais aucun enregistrement exploitable n’a été trouvé dans results.');
            }

            window.allData = allData;
            submissionTimelineSourceData = allData;
            lastKoboFetchMeta = {
                providers: [providerName || 'Méthode inconnue'],
                pages: 1,
                expectedCount: Number.isFinite(Number(payload?.count)) ? Number(payload.count) : allData.length
            };

            // Réinjecte le jeu Kobo dans TOUTES les vues de la version actuelle.
            ...
            $('#sync-status')
                .html(`<span class="badge bg-success sync-badge"><i class="fas fa-check-double"></i> Ok : ${allData.length} Lignes</span>`)
                .append(`<span class="badge bg-info text-dark ms-2"><i class="fas fa-route"></i> ${escapeRealisationHtml(providerName || 'Méthode inconnue')}</span>`)
                .append(dictionaryBadge)
                .append('<span class="badge bg-primary ms-2"><i class="fas fa-cloud"></i> KoboToolbox</span>');

            if (typeof updateDashboardDataSourceUi === 'function') {
                const countText = Number.isFinite(Number(payload?.count))
                    ? `${allData.length.toLocaleString('fr-FR')} affiché(s) sur ${Number(payload.count).toLocaleString('fr-FR')} annoncé(s) par Kobo`
                    : `${allData.length.toLocaleString('fr-FR')} enregistrement(s)`;
                updateDashboardDataSourceUi(
                    'kobo',
                    `<i class="fas fa-check-circle"></i> <strong>Source active :</strong> KoboToolbox — ${countText} via <strong>${escapeRealisationHtml(providerName || 'Méthode inconnue')}</strong>.`
                );
            }
            ...
            const message = error instanceof Error ? error.message : String(error ?? 'Erreur inconnue');
            const isNetworkError = /connexion|réseau|network|fetch|bloque|cors|antivirus|adblock|ublock|proxy|allorigins|codetabs/i.test(message);
            const title = isNetworkError ? 'Erreur de connexion KoboToolbox' : 'Erreur interne de traitement';
            console.error(title + ' :', error);
            $('#error-box').html(`<strong>${title} :</strong> ${escapeRealisationHtml(message)}`).show();
            $('#sync-status').html(`<span class="badge bg-danger sync-badge">${isNetworkError ? 'Échec Kobo' : 'Erreur de traitement'}</span>`);
