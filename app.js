...
const realisationComparisonPointLabelsPlugin = {
    ...
    afterDatasetsDraw(chart, args, options) {
        if (!options || options.display === false || !chart.chartArea) return;
        const ctx = chart.ctx;
    ...
    afterDatasetsDraw(chart, args, options) {
        ...
        chart.data.datasets.forEach((dataset, datasetIndex) => {
            ...
            (meta.data || []).forEach((bar, index) => {
                ...
                ctx.fillStyle = width > ctx.measureText(valueText).width + 14 ? '#fff' : (dataset.borderColor || '#333');
                ctx.textAlign = width > ctx.measureText(valueText).width + 14 ? 'center' : 'left';
                ctx.fillText(valueText, width > ctx.measureText(valueText).width + 14 ? left + width / 2 : right + 5, bar.y);
                ctx.fillStyle = '#4d5d53';
                ctx.font = '500 8px Arial';
                ctx.textAlign = 'left';
                ctx.fillText(formatRealisationComparisonDate(record.dateStart), Math.max(chart.chartArea.left, left), bar.y - 12);
                ctx.textAlign = 'right';
                ctx.fillText(formatRealisationComparisonDate(record.dateEnd), Math.min(chart.chartArea.right, right), bar.y - 12);
                ctx.font = '600 9px Arial';
            });
        });
        ctx.restore();
...
function renderRealisationComparisonTimeline(scope, records) {
    ...
    const width = Math.min(16000, Math.max(1050, 650 + durationDays * 18));
    const height = Math.min(14000, Math.max(430, 145 + complete.length * 48));
    setRealisationComparisonStageSize(stage, scroll, width, height);

    const labels = complete.map(record => `${record.activite} — ${getRealisationComparisonLabel(record, scopeKey === 'STD')}`);
    const levels = scopeKey === 'STD' ? ['DREN', 'CISCO', 'ZAP'] : [scopeKey];
    ...
    const chart = new Chart(canvas.getContext('2d'), {
        ...
        options: {
            ...
            normalized: true,
            indexAxis: 'y',
            layout: { padding: { right: 175, top: 18, bottom: 18 } },
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        title: items => items?.[0] ? labels[items[0].dataIndex] : 'Réalisation',
                        label: context => {
            ...
            scales: {
                x: {
                    ...
                    ticks: { maxTicksLimit: 16, callback: value => formatRealisationGanttDate(value) },
                    grid: { color: 'rgba(70, 100, 82, .12)' }
                },
                y: {
                    stacked: true,
                    ...
                    ticks: {
                        autoSkip: false,
                        font: { size: complete.length > 25 ? 8 : 9 },
...
window.runRealisationComparison = function(scope) {
    ...
    if (settings.start && settings.end && settings.start > settings.end) {
        if (status) {
            ...
            status.innerHTML = '<i class="fas fa-exclamation-triangle"></i> La date de début minimale doit être antérieure ou égale à la date de fin maximale.';
        }
        return;
    }
    ...
    if (status) {
        ...
        status.innerHTML = records.length
            ? `<i class="fas fa-check-circle"></i> Analyse terminée pour <strong>${escapeRealisationHtml(selectedActivity)}</strong> : ${records.length.toLocaleString('fr-FR')} réalisation(s), seuil de proximité ${settings.tolerance.toFixed(0)} %.`
            : '<i class="fas fa-info-circle"></i> Aucune réalisation ne correspond aux critères choisis.';
    }
};
...
function getDisplayedRealisationRecords(level, records, settings, forcedEntities = null) {
    ...
    return levelRecords
        ...
        .sort((a, b) => {
            ...
            return String(a.activite || '').localeCompare(String(b.activite || ''), 'fr');
        });
}
...
function renderRealisationDetailTable(level, records, settings, forcedEntities = null) {
    ...
    if (!displayed.length) {
        tbody.innerHTML = '<tr><td colspan="12" class="text-center text-muted py-3">Aucune réalisation à afficher dans le tableau.</td></tr>';
    ...
    tbody.innerHTML = displayed.map((record, index) => {
        ...
        const status = record.isAnomaly
            ? '<span class="badge bg-warning text-dark">Sous-activité à vérifier</span>'
        ...
        return `<tr>
            ...
            <td title="${escapeRealisationHtml(record.activite || 'Non spécifiée')}">${escapeRealisationHtml(record.activite || 'Non spécifiée')}</td>
...
function getRealisationGanttStageHeight(rowCount, individual = false) {
    ...
    const rowHeight = individual ? 76 : 82;
