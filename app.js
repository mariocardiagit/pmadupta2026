...
var realisationTimelineChartsRefs = { DREN: null, CISCO: null, ZAP: null };
...
function renderRealisationLegend(level, datasets) {
    const target = document.getElementById(`real-legend-${level.toLowerCase()}`);
    if (!target) return;
    if (!datasets.length) { target.innerHTML = ''; return; }
    target.innerHTML = datasets.map(dataset => `<span class="realisation-legend-item"><i style="background:${dataset.borderColor}"></i><strong>${escapeRealisationHtml(dataset.label)}</strong><small>${Number(dataset._total || 0).toLocaleString('fr-FR')}</small></span>`).join('');
}

function renderRealisationTimelineLevel(level, records, periodKeys, settings) {
    const canvas = document.getElementById(`realisationTimelineChart${level}`);
    const empty = document.getElementById(`real-empty-${level.toLowerCase()}`);
    const stage = document.getElementById(`real-stage-${level.toLowerCase()}`);
    if (!canvas || !empty || !stage) return;
    if (realisationTimelineChartsRefs[level]) realisationTimelineChartsRefs[level].destroy();
    const byEntity = {};
    records.filter(item => item.niveau === level).forEach(item => {
    ...
    let ranked = Object.entries(byEntity).map(([entity, values]) => [entity, Object.values(values).reduce((sum, value) => sum + value, 0)]).sort((a,b) => b[1] - a[1] || a[0].localeCompare(b[0], 'fr'));
    if (settings.top !== 'all') ranked = ranked.slice(0, Number(settings.top));
    if (!ranked.length || !periodKeys.length) {
        canvas.style.display = 'none'; empty.style.display = 'flex'; renderRealisationLegend(level, []); return;
    }
    canvas.style.display = 'block'; empty.style.display = 'none';
    const labels = periodKeys.map(key => formatRealisationPeriodLabel(key, settings.granularity));
    const datasets = ranked.map(([entity, total], index) => {
        ...
        return {
            ...
            _total: total,
            borderColor: color,
            backgroundColor: getSubmissionTimelineColor(index, settings.chartType === 'area' ? .18 : .48),
            pointBackgroundColor: color,
            pointBorderColor: '#fff', pointBorderWidth: 1,
            pointRadius: periodKeys.length > 45 ? 0 : 3,
            pointHoverRadius: 5, borderWidth: 2, tension: .22,
            fill: settings.chartType === 'area'
        };
    });
    const isBar = settings.chartType === 'bar' || settings.chartType === 'stackedBar';
    const stacked = settings.chartType === 'stackedBar';
    stage.style.minWidth = `${Math.max(900, 340 + periodKeys.length * 72)}px`;
    stage.style.height = `${Math.min(760, Math.max(410, 300 + ranked.length * 17))}px`;
    realisationTimelineChartsRefs[level] = new Chart(canvas.getContext('2d'), {
        type: isBar ? 'bar' : 'line',
        ...
        options: {
            ...
            interaction: { mode: 'index', intersect: false },
            layout: { padding: { right: isBar ? 20 : 245, top: 12, bottom: 6, left: 4 } },
            plugins: {
                legend: { display: false },
                realisationEndLabelsPlugin: { enabled: !isBar },
                tooltip: { callbacks: { label: context => `${context.dataset.label} : ${Number(context.parsed.y || 0).toLocaleString('fr-FR')}` } }
            },
            scales: {
                x: { stacked, title: { display: true, text: 'Date de réalisation / période', font: { weight: 'bold' } }, ticks: { maxRotation: isBar ? 35 : 0, autoSkip: true, maxTicksLimit: 18 } },
                y: { stacked, beginAtZero: true, title: { display: true, text: settings.mode === 'cumulative' ? 'Réalisations cumulées' : 'Réalisations de la période', font: { weight: 'bold' } } }
            }
        }
    });
    renderRealisationLegend(level, datasets);
}
...
window.runRealisationTemporel = function() {
    ...
    const dated = allRecords.filter(item => {
        ...
        if (settings.start && date < settings.start) return false;
        if (settings.end && date > settings.end) return false;
        return true;
    });
    const periodKeys = [...new Set(dated.map(item => getRealisationPeriodKey(getRealisationReferenceDate(item, settings.reference), settings.granularity)))].sort((a,b) => realisationPeriodSortValue(a, settings.granularity) - realisationPeriodSortValue(b, settings.granularity));
    ...
    if (status) {
        const missing = allRecords.length - allRecords.filter(item => getRealisationReferenceDate(item, settings.reference)).length;
        status.classList.toggle('d-none', missing === 0 && dated.length > 0);
        status.innerHTML = dated.length === 0 ? '<i class="fas fa-exclamation-triangle"></i> Aucune réalisation ne possède la date choisie dans la période sélectionnée.' : `<i class="fas fa-info-circle"></i> ${missing} réalisation(s) sans la date de référence choisie ont été écartées.`;
    }
...
window.resetRealisationTimelineControls = function() {
    $('#real-period-select').val('month'); $('#real-display-mode').val('detailed'); $('#real-date-reference').val('start'); $('#real-chart-type').val('line'); $('#real-top-entities').val('10'); $('#real-date-start, #real-date-end').val('');
    window.runRealisationTemporel();
};

window.exportRealisationChart = function(level) {
    const chart = realisationTimelineChartsRefs[String(level || '').toUpperCase()];
    if (!chart) return alert('Aucun graphique disponible pour ce niveau.');
    const link = document.createElement('a');
    link.download = `realisations_${String(level).toLowerCase()}_${new Date().toISOString().slice(0,19).replace(/[-:T]/g,'')}.png`;
    link.href = chart.toBase64Image('image/png', 1);
    link.click();
};

window.exportAllRealisationCharts = async function() {
    const available = Object.entries(realisationTimelineChartsRefs).filter(([, chart]) => chart);
    if (!available.length) return alert('Actualisez d’abord l’analyse temporelle.');
    if (typeof JSZip === 'undefined') { available.forEach(([level]) => window.exportRealisationChart(level)); return; }
    const zip = new JSZip();
    available.forEach(([level, chart]) => {
        const base64 = chart.toBase64Image('image/png', 1).split(',')[1];
        zip.file(`realisations_${level.toLowerCase()}.png`, base64, { base64: true });
    });
    const settings = getRealisationTimelineSettings();
...
$(document).ready(function() {
    ...
    if (realisationMasterTab) {
        realisationMasterTab.addEventListener('shown.bs.tab', function() {
            window.runRealisationTemporel();
            setTimeout(function() {
                Object.values(realisationTimelineChartsRefs).forEach(chart => { if (chart) chart.resize(); });
            }, 80);
        });
    }
});
