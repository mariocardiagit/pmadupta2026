...
async function runRealisationSemanticComparison(scope, records, settings, force = false) {
    ...
    try {
        ...
        const semanticChartCard = document.getElementById(`real-compare-scatter-${key}`)?.closest('.real-comparison-chart-card');
        const semanticChartSmall = semanticChartCard?.querySelector('.real-comparison-chart-heading small');
        if (semanticChartSmall) semanticChartSmall.textContent = `Axe X : valeur numérique · Axe Y : thématique Sentence-BERT de ${getRealisationComparisonDimensionConfig(settings.dimension).label.toLowerCase()} · Point : entité responsable`;
