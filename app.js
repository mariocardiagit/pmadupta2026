...
const submissionLineEndLabelsPlugin = {
    ...
    afterDatasetsDraw(chart, args, pluginOptions) {
        ...
        const desiredX = Math.min((chartArea.right || 0) + 10, (chart.width || 0) - 6);
        const labels = [];
        ...
        const minGap = 18;
        const topBound = (chartArea.top || 0) + 8;
        const bottomBound = (chartArea.bottom || 0) - 8;

        for (let i = 0; i < labels.length; i++) {
            labels[i].y = Math.max(labels[i].y, topBound);
            if (i > 0 && labels[i].y - labels[i - 1].y < minGap) {
                labels[i].y = labels[i - 1].y + minGap;
            }
        }
        for (let i = labels.length - 1; i >= 0; i--) {
            labels[i].y = Math.min(labels[i].y, bottomBound);
        ...
        labels.forEach(item => {
            ...
            ctx.lineWidth = 1.5;
            ctx.beginPath();
            ctx.moveTo(Math.min(item.anchorX + 4, chartArea.right), item.y);
            ctx.lineTo(Math.max(chartArea.right + 4, item.anchorX + 4), item.y);
            ctx.lineTo(boxX, item.y);
            ctx.stroke();
            ...
            ctx.strokeStyle = item.color;
            ctx.lineWidth = 1;
            ctx.strokeRect(boxX, boxY - boxH / 2, boxW, boxH);
            ctx.fillStyle = item.color;
            ctx.fillText(labelText, boxX + paddingX, boxY + 0.5);
        });
        ctx.restore();
    }
};
...
function renderOneSubmissionTimelineChart(level, datedRows, bucketKeys, granularity, topLimit) {
    ...
    submissionTimelineChartsRefs[level] = new Chart(canvas.getContext('2d'), {
        ...
        options: {
            ...
            plugins: {
                submissionLineEndLabelsPlugin: { display: true },
                legend: {
                    position: 'bottom',
