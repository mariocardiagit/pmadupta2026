...
function updateSecureKoboAttachmentServiceStatus() {
    ...
    status.innerHTML = proxyUrl
        ? `<span class="text-success"><i class="fas fa-shield-alt"></i> <strong>Cloudflare Workers sécurisé configuré :</strong> <code>${escapeRealisationHtml(proxyUrl)}</code>. Le token Kobo reste uniquement côté serveur. Utilisez « Tester » pour vérifier la chaîne complète avant une sauvegarde Base64.</span>`
        : '<span class="text-warning"><i class="fas fa-cloud"></i> <strong>Cloudflare Workers Free est le service gratuit recommandé par défaut, mais il n’est pas encore configuré.</strong> Une URL ne peut pas être universelle car le Worker doit contenir votre propre token Kobo comme secret. Sans configuration, les pièces jointes protégées peuvent renvoyer HTTP 403 et la sauvegarde sera marquée PARTIELLE.</span>';
}
...
window.testSecureKoboAttachmentService = async function() {
    ...
    try {
        ...
        if (!sample) {
            if (test) test.innerHTML = `<span class="text-success"><i class="fas fa-check-circle"></i> <strong>Worker opérationnel.</strong> Service : ${escapeRealisationHtml(health.service || 'Cloudflare Workers')} · token serveur configuré. Aucun fichier joint actuellement chargé pour effectuer le test de téléchargement de bout en bout.</span>`;
            return true;
        }
        ...
        if (test) test.innerHTML = `<span class="text-success"><i class="fas fa-check-circle"></i> <strong>TEST COMPLET RÉUSSI.</strong> Cloudflare Worker accessible, secret Kobo configuré et pièce jointe Kobo récupérée avec authentification (${probeBlob.size.toLocaleString('fr-FR')} octets · ${escapeRealisationHtml(contentType)}). La sauvegarde Base64 peut viser 100 % des pièces jointes accessibles.</span>`;
        return true;
    } catch (error) {
