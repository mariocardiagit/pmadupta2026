...
function getRealisationAggregationCompositionText(record, dimension) {
    ...
    const detail = items.map(item => {
        const sourceNote = dimension === 'activite' && item.sourceCount
            ? ` ; ${item.sourceCount} Sous Activité${item.sourceCount > 1 ? 's' : ''}`
            : '';
        const statusNote = item.status?.length ? ` ; ${item.status.join(' / ')}` : '';
        return `${item.index}. ${item.label} = ${item.valeur.toLocaleString('fr-FR')} (${item.jours} jour${item.jours > 1 ? 's' : ''}${sourceNote}${statusNote})`;
    }).join(' ; ');
    return `${items.length} ${plural} : ${detail}`;
}
...
function getRealisationAggregationCompositionHtml(record, dimension) {
    ...
    const details = items.map(item => {
        ...
        const status = item.status?.length
            ...
            : '';
        return `<div class="realisation-composition-item"><strong>${item.index}.</strong> ${escapeRealisationHtml(item.label)} <span class="text-nowrap">= <strong>${item.valeur.toLocaleString('fr-FR')}</strong> · ${item.jours} jour${item.jours > 1 ? 's' : ''}${escapeRealisationHtml(sourceNote)}</span>${status}</div>`;
    }).join('');
...
function getRealisationWordTable(level) {
    ...
    if (!table) return { headers: [], rows: [], title: `Tableau détaillé des réalisations affichées — ${key}` };
    const headerCells = Array.from(table.querySelectorAll('thead th'));
    const indexes = [1,2,3,4,5,6,7,11,12].filter(index => index < headerCells.length);
    const headers = indexes.map(index => String(headerCells[index]?.textContent || '').replace(/\s+/g,' ').trim());
    const rows = Array.from(table.querySelectorAll('tbody tr')).map(tr => {
        const cells = Array.from(tr.querySelectorAll('td'));
        return indexes.map(index => String(cells[index]?.textContent || '').replace(/\s+/g,' ').trim());
    }).filter(row => row.some(value => value && !/Aucune réalisation/i.test(value)));
    return { headers, rows, title: `Tableau détaillé des réalisations affichées — ${key}` };
}
