...
function getSubmissionTimelineColor(index, alpha) {
    ...
    return `hsla(${hue}, 68%, 40%, ${alpha})`;
}

const submissionTimelineLinePatterns = [
    [14, 5], [5, 4], [11, 3, 2, 3], [2, 5], [8, 4],
    [16, 3, 3, 3], [6, 2], [3, 3, 9, 3], [12, 6], [1, 4]
];
const submissionTimelinePointStyles = ['circle', 'rectRot', 'triangle', 'rect', 'star', 'crossRot', 'cross', 'dash', 'line', 'circle'];

// Affiche le nom de chaque entité à côté de son dernier point réellement soumis.
// Les libellés sont espacés verticalement afin de rester lisibles lorsque plusieurs
// courbes se terminent au même niveau.
const submissionTimelineEndLabelsPlugin = {
    id: 'submissionTimelineEndLabels',
    afterDatasetsDraw(chart, args, pluginOptions) {
        ...
        chart.data.datasets.forEach((dataset, datasetIndex) => {
            ...
            if (lastPositiveIndex < 0) return;

            const point = chart.getDatasetMeta(datasetIndex).data[lastPositiveIndex];
            if (!point || !Number.isFinite(point.x) || !Number.isFinite(point.y)) return;

            const fullText = String(dataset.label || '');
            const maxChars = pluginOptions.maxChars || 22;
            const text = fullText.length > maxChars ? fullText.slice(0, maxChars - 1) + '…' : fullText;
            labels.push({
                datasetIndex,
                pointX: point.x,
                pointY: point.y,
        ...
        const minGap = pluginOptions.minGap || 18;
        const topLimit = chartArea.top + 10;
        const bottomLimit = chartArea.bottom - 10;

        labels[0].y = Math.max(labels[0].y, topLimit);
        ...
        const labelX = chartArea.right + 18;
        labels.forEach(label => {
            ...
            // Trait de liaison entre la courbe et son libellé.
            ctx.beginPath();
            ctx.moveTo(Math.min(label.pointX + 5, chartArea.right), label.pointY);
            ctx.lineTo(chartArea.right + 7, label.y);
            ctx.stroke();

            // Échantillon visuel de la série, identique au style de sa courbe.
            ctx.beginPath();
            ctx.moveTo(labelX, label.y);
            ctx.lineTo(labelX + 18, label.y);
            ctx.stroke();
            ctx.setLineDash([]);
            ctx.beginPath();
            ctx.arc(labelX + 9, label.y, 2.4, 0, Math.PI * 2);
            ctx.fill();

            ctx.fillStyle = '#2c3e50';
            ctx.fillText(label.text, labelX + 23, label.y);
            ctx.restore();
        });
        ctx.restore();
    }
...
function renderOneSubmissionTimelineChart(level, datedRows, bucketKeys, granularity, topLimit) {
    ...
    let datasets = rankedEntities.map(([entity], index) => {
        let color = getSubmissionTimelineColor(index, 0.82);
        let pointColor = getSubmissionTimelineColor(index, 1);
        return {
            label: entity,
            data: bucketKeys.map(bucket => valuesByEntity[entity][bucket] || 0),
            borderColor: color,
            backgroundColor: getSubmissionTimelineColor(index, 0.16),
            pointBackgroundColor: getSubmissionTimelineColor(index, 0.22),
            pointBorderColor: pointColor,
            pointBorderWidth: 2,
            pointStyle: submissionTimelinePointStyles[index % submissionTimelinePointStyles.length],
            pointRadius: bucketKeys.length > 45 ? 0 : 3.2 + (index % 2),
            pointHoverRadius: 6,
            borderWidth: 2.8,
            borderDash: submissionTimelineLinePatterns[index % submissionTimelineLinePatterns.length],
            borderDashOffset: index * 1.7,
    ...
    submissionTimelineChartsRefs[level] = new Chart(canvas.getContext('2d'), {
        ...
        plugins: [submissionTimelineEndLabelsPlugin],
        options: {
            responsive: true,
            maintainAspectRatio: false,
            normalized: true,
            layout: {
                padding: { right: window.innerWidth >= 768 ? 175 : 8 }
            },
            interaction: { mode: 'index', intersect: false },
            plugins: {
                submissionTimelineEndLabels: {
                    ...
                    labels: {
                        ...
                        generateLabels(chart) {
                            ...
                            return defaultLabels.map((item, index) => ({
                                ...
                                pointStyle: submissionTimelinePointStyles[index % submissionTimelinePointStyles.length],
                                lineWidth: 3,
                                lineDash: submissionTimelineLinePatterns[index % submissionTimelineLinePatterns.length]
                            }));
                        }
                    }
                },
