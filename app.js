...
function updateSecureKoboAttachmentServiceStatus() {
    ...
    status.innerHTML = proxyUrl
        ? `<span class="text-success"><i class="fas fa-shield-alt"></i> Service sécurisé configuré : <code>${escapeRealisationHtml(proxyUrl)}</code>. Le token Kobo reste uniquement côté serveur.</span>`
        : '<span class="text-warning"><i class="fas fa-exclamation-triangle"></i> Aucun service sécurisé configuré. Les pièces jointes protégées Kobo peuvent renvoyer HTTP 403 ; une sauvegarde partielle sera alors clairement signalée.</span>';
}
