...
async function fetchData() {
    ...
    try {
        ...
        if (!fetchSuccess) {
            throw new Error("La connexion directe à KoboToolbox a échoué. Vérifiez l’accès public aux soumissions ou utilisez la source Fichier JSON.");
        }
...
async function fetchData() {
    ...
    try {
        ...
        if (!fetchSuccess) {
            throw new Error("Connexion directe KoboToolbox impossible. Vérifiez que l’asset autorise la lecture anonyme des soumissions ou importez un fichier JSON.");
        }
...
async function fetchKoboJsonDirect(url) {
    ...
    try {
        ...
        throw new Error(`Connexion directe KoboToolbox impossible : ${error?.message || error}`);
    }

    if (!response.ok) {
        if (response.status === 401 || response.status === 403) {
            throw new Error(`HTTP ${response.status} — l’asset KoboToolbox n’autorise pas la lecture anonyme des soumissions. Cette version fonctionne uniquement par fetch() direct ; activez l’accès public/AnonymousUser pour la consultation des soumissions ou utilisez la source Fichier JSON.`);
        }
...
async function fetchCompleteKoboDatabase() {
    ...
    while (nextUrl) {
        ...
        const payload = await fetchKoboJsonDirect(normalizedUrl);
