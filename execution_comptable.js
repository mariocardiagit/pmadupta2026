...
(function () {
    ...
    function findCompanionLabelField(codeField) {
        ...
        for (const candidate of candidates) {
            ...
            let score = 0;
            if (words.some(w => generic.has(w))) score += 7;
            ...
            if (a && b && a === b) score += 3;
            if (/(code|uid|identifiant|numero|num)/i.test(fieldLabel(candidate))) score -= 4;
            const paired = state.flat.slice(0,400).map(r => [clean(r[codeField]), clean(r[candidate])]).filter(([c,l]) => c && l && c !== l);
            if (paired.length) {
                const longer = paired.filter(([c,l]) => l.length > c.length + 2).length / paired.length;
                score += longer >= .6 ? 3 : 0;
    ...
    function displayCodeAndLabel(row, codeField, labelField = '') {
        ...
        const label = resolved ? clean(row?.[resolved]) : '';
    ...
    function populateFieldControls() {
        ...
        const autoLabel=findCompanionLabelField(autoEntity) || detectField([/(libelle|libellé|label|intitule|intitulé|designation|désignation|nom.*activite|nom.*activité|description)/i],['text','category']) || '';
