/* ============================================================================
   EXECUTION_COMPTABLE — V7.0 — 2026-08-28
...
(function () {
    ...
    function normalizeSourceInput(input) {
        ...
        const m=url.pathname.match(/\/api\/v2\/assets\/([^/]+)/) || url.pathname.match(/\/assets\/([^/]+)/) || url.pathname.match(/\/forms\/([^/?#]+)/);
    ...
    function wireEvents(){
        ...
        byId('execution-comptable-tab')?.addEventListener('shown.bs.tab',()=>setTimeout(()=>Object.values(state.charts).forEach(c=>{try{c?.resize();}catch(_){}}),50));
    }

    function init(){if(!byId('execution-comptable'))return;const saved=localStorage.getItem(STORAGE_KEY)||'';if(byId('exec-kobo-source'))byId('exec-kobo-source').value=saved;wireEvents();state.filtered=[];refreshKPIs();renderDataTable();if(saved)setStatus('<i class="fas fa-info-circle"></i> Une source EXECUTION_COMPTABLE est enregistrée dans ce navigateur. Cliquez sur « Connecter / Actualiser ».');}
