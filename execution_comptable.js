/* ============================================================================
   EXECUTION_COMPTABLE — V7.5 — 2026-08-28
...
(function () {
    ...
    const state = {
        ...
        charts: {}, expertAlerts: [], riskRows: [], sourceUrl: '', labelCompanions: new Map(), sbertExtractor: null,
        dictionaryExact: new Map(), dictionaryGeneric: new Map(), dictionaryListByQuestion: new Map(), dictionaryQuestionLabels: new Map(),
        dictionaryReady: false, dictionaryPromise: null,
    ...
    function fieldLabel(field) {
        ...
        const candidates = [raw.toLowerCase(), short.toLowerCase(), normalizeDictionaryKey(raw), normalizeDictionaryKey(short)];
        for (const key of candidates) {
            const local = state.dictionaryQuestionLabels.get(key);
            if (local) return local;
        }
        try {
            const hm = window.headerMap || {};
            const direct = hm[raw] || hm[short] || hm[raw.toLowerCase()] || hm[short.toLowerCase()];
            if (direct) return clean(direct);
        } catch (_) {}
        return short.replace(/_/g,' ').replace(/\s+/g,' ').trim() || raw;
    }
    function normalizeFieldWords(value) {
        return clean(value).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')
            .replace(/[^a-z0-9]+/g,' ').trim().split(/\s+/).filter(Boolean);
    }
    function isLikelyCodeValue(value) {
        const text = clean(value);
        if (!text) return false;
        if (/^\d{1,8}$/.test(text)) return true;
        if (/^[A-Z]{1,6}[-_ ]?\d{1,8}[A-Z]?$/.test(text.toUpperCase())) return true;
        if (/^(A|P|SA|SP|DREN|CISCO|ZAP)[-_ ]?\d+/i.test(text)) return true;
        return false;
    }
    function isLikelyMachineIdentifier(value) {
        const text = clean(value);
        if (!text) return false;
        if (isLikelyCodeValue(text)) return true;
        if (/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(text)) return true;
        if (/^(uuid:)?[0-9a-f]{24,}$/i.test(text)) return true;
        if (/^[A-Za-z0-9_-]{16,}$/.test(text) && /[A-Z]/.test(text) && /[a-z]/.test(text) && /\d/.test(text)) return true;
        if (/^[A-Za-z0-9_-]{22,}$/.test(text) && !/\s/.test(text)) return true;
        return false;
    }
    async function loadExecutionDictionary() {
        if (state.dictionaryPromise) return state.dictionaryPromise;
        state.dictionaryPromise = (async () => {
            try {
                if (typeof XLSX === 'undefined') throw new Error('Bibliothèque XLSX non disponible');
                const response = await fetch('dictionnaire.xlsx?v=20260828-v7-4', { cache: 'no-store' });
                if (!response.ok) throw new Error(`HTTP ${response.status}`);
                const workbook = XLSX.read(new Uint8Array(await response.arrayBuffer()), { type: 'array' });
                const exact = new Map(), labelsByCode = new Map(), listByQuestion = new Map(), questionLabels = new Map();
                if (workbook.SheetNames.includes('survey')) {
                    XLSX.utils.sheet_to_json(workbook.Sheets['survey'], { defval: '' }).forEach(row => {
                        const name = clean(row.name), type = clean(row.type), label = clean(row.label || row['label::Français (fr)'] || row['label::French (fr)']);
                        if (!name) return;
                        const short = name.split('/').pop();
                        const keys = [name.toLowerCase(), short.toLowerCase(), normalizeDictionaryKey(name), normalizeDictionaryKey(short)];
                        if (label) keys.forEach(k => questionLabels.set(k, label));
                        const match = type.match(/^select_(?:one|multiple)\s+([^\s]+)/i);
                        if (match) keys.forEach(k => listByQuestion.set(k, clean(match[1]).toLowerCase()));
                    });
                }
                if (workbook.SheetNames.includes('choices')) {
                    XLSX.utils.sheet_to_json(workbook.Sheets['choices'], { defval: '' }).forEach(row => {
                        const list = clean(row.list_name).toLowerCase(), code = clean(row.name), label = clean(row.label || row['label::Français (fr)'] || row['label::French (fr)']);
                        if (!code || !label || label.toLowerCase() === code.toLowerCase()) return;
                        const codeKey = code.toLowerCase();
                        if (list) exact.set(`${list}::${codeKey}`, label);
                        if (!labelsByCode.has(codeKey)) labelsByCode.set(codeKey, new Set());
                        labelsByCode.get(codeKey).add(label);
                    });
                }
                const generic = new Map();
                labelsByCode.forEach((set, code) => {
                    const vals = [...set];
                    if (vals.length === 1) generic.set(code, vals[0]);
                    else {
                        const norm = new Set(vals.map(v => normalizeDictionaryKey(v)));
                        if (norm.size === 1) generic.set(code, vals[0]);
                    }
                });
                state.dictionaryExact = exact;
                state.dictionaryGeneric = generic;
                state.dictionaryListByQuestion = listByQuestion;
                state.dictionaryQuestionLabels = questionLabels;
                state.dictionaryReady = true;
                return true;
            } catch (error) {
                console.warn('EXECUTION_COMPTABLE: dictionnaire local non chargé', error);
    ...
    function resolveDictionaryListName(codeField) {
        const full = String(codeField || '');
        const short = full.split('/').pop();
        const keys = [full.toLowerCase(), short.toLowerCase(), normalizeDictionaryKey(full), normalizeDictionaryKey(short)];
        for (const key of keys) {
            const direct = state.dictionaryListByQuestion.get(key);
            if (direct) return direct;
        }
        const target = normalizeDictionaryKey(short).replace(/index$/,'');
        for (const [key, list] of state.dictionaryListByQuestion.entries()) {
            const k = normalizeDictionaryKey(key).replace(/index$/,'');
            if (k && target && (k === target || k.includes(target) || target.includes(k))) return list;
        }
        return '';
    }
    function lookupDictionaryLabel(codeField, rawCode) {
        const code = clean(rawCode);
        if (!code) return '';
        const codeKey = code.toLowerCase();
        const localList = resolveDictionaryListName(codeField);
        const localExact = localList ? clean(state.dictionaryExact.get(`${localList}::${codeKey}`)) : '';
        if (localExact && localExact.toLowerCase() !== codeKey) return localExact;
        const localGeneric = clean(state.dictionaryGeneric.get(codeKey));
        if (localGeneric && localGeneric.toLowerCase() !== codeKey) return localGeneric;
        try {
            const qMap = window.questionListMap || {};
            const vMap = window.valueMap || {};
            const ext = window.externalDict || {};
            const fullField = String(codeField || '').toLowerCase();
            const shortField = fullField.split('/').pop();
            let listName = qMap[fullField] || qMap[shortField] || '';
            if (!listName) {
                const match = Object.keys(qMap).find(k => k === shortField || shortField.includes(k) || k.includes(shortField));
                if (match) listName = qMap[match] || '';
            }
            const exact = listName ? clean(vMap[`${String(listName).toLowerCase()}::${codeKey}`]) : '';
            if (exact && exact.toLowerCase() !== codeKey) return exact;
            const fallback = clean(ext[codeKey]);
            if (fallback && fallback.toLowerCase() !== codeKey && !isLikelyMachineIdentifier(fallback)) return fallback;
        } catch (_) {}
        return '';
    }
    function findCompanionLabelField(codeField) {
        if (!codeField) return '';
        if (state.labelCompanions.has(codeField)) return state.labelCompanions.get(codeField) || '';
        const codeWords = normalizeFieldWords(`${codeField} ${fieldLabel(codeField)}`).filter(w => !['code','id','uid','numero','num','n'].includes(w));
        const generic = new Set(['libelle','label','intitule','nom','designation','description','texte','text']);
        const candidates = state.fields.filter(f => f !== codeField && ['text','category'].includes(state.meta.get(f)?.type));
        let best = '', bestScore = -Infinity;
        for (const candidate of candidates) {
            const words = normalizeFieldWords(`${candidate} ${fieldLabel(candidate)}`);
            let score = 0;
            const hasLabelWord = words.some(w => generic.has(w));
            if (hasLabelWord) score += 9;
            const shared = [...new Set(codeWords.filter(w => words.includes(w) && !generic.has(w)))];
            score += shared.length * 2;
            const a = codeField.split('/').slice(0,-1).join('/').toLowerCase();
            const b = candidate.split('/').slice(0,-1).join('/').toLowerCase();
            if (a && b && a === b) score += 2;
            if (/(code|uid|uuid|identifiant|numero|num|index|instance|submission|meta|row|produit|sous.?produit)/i.test(`${candidate} ${fieldLabel(candidate)}`) && !hasLabelWord) score -= 10;
            const paired = state.flat.slice(0,400).map(r => [clean(r[codeField]), clean(r[candidate])]).filter(([c,l]) => c && l && c !== l);
            if (paired.length) {
                const codeLikeRatio = paired.filter(([,l]) => isLikelyCodeValue(l) || isLikelyMachineIdentifier(l)).length / paired.length;
                const descriptiveRatio = paired.filter(([c,l]) => !isLikelyCodeValue(l) && l.length > Math.max(6, c.length + 2)).length / paired.length;
                if (codeLikeRatio >= .50) score -= 12;
                if (descriptiveRatio >= .50) score += 5;
                const map = new Map(); let conflicts = 0;
                paired.forEach(([c,l]) => { if (map.has(c) && map.get(c) !== l) conflicts++; else if (!map.has(c)) map.set(c,l); });
                if (map.size >= 1 && conflicts / paired.length <= .1) score += 2;
            }
            if (score > bestScore) { bestScore = score; best = candidate; }
        }
        const result = bestScore >= 8 ? best : '';
        state.labelCompanions.set(codeField, result);
        return result;
    }
    function displayCodeAndLabel(row, codeField, labelField = '') {
        if (!codeField) return 'Non renseigné';
        const code = clean(row?.[codeField]) || 'Non renseigné';
        const dictionaryLabel = lookupDictionaryLabel(codeField, code);
        // Règle V7.4 : le dictionnaire XLSForm est prioritaire sur toute heuristique de champ compagnon.
        if (dictionaryLabel && dictionaryLabel !== code && !isLikelyMachineIdentifier(dictionaryLabel)) {
            return dictionaryLabel.startsWith(code) ? dictionaryLabel : `${code} — ${dictionaryLabel}`;
        }
        const resolved = labelField || findCompanionLabelField(codeField);
        let label = resolved ? clean(row?.[resolved]) : '';
        if (!label || label === code || isLikelyMachineIdentifier(label) || isLikelyCodeValue(label)) label = '';
        if (!label || code.includes(label)) return code;
        if (label.startsWith(code)) return label;
        return `${code} — ${label}`;
    }
    function graphTitle(text) {
        return { display: true, text, padding: { top: 4, bottom: 14 }, font: { size: 15, weight: 'bold' } };
    }
    function wrapChartLabel(value, max = 34) {
        const text = clean(value);
        ...
        const out = code ? [code, ...lines] : lines;
        return out.slice(0, 3);
    }

    function isTechnicalAnalysisField(field) {
        const raw = clean(field).toLowerCase();
        const leaf = raw.split('/').pop().replace(/^_+/, '');
        const label = clean(fieldLabel(field)).toLowerCase();
        const joined = `${raw} ${label}`;

        // Métadonnées/indices Kobo ou champs techniques qui ne représentent pas une mesure comptable.
        if (['row','index','id','uid','uuid','instanceid','instance_id','submissionid','submission_id','version','xform_id_string'].includes(leaf)) return true;
        if (/^(?:__row|_row|_index|_id|_uuid|_version|_xform_id_string)$/i.test(raw)) return true;
        if (raw.includes('formhub/uuid') || raw.includes('meta/instanceid')) return true;
        if (/(^|[\s_\-/])(row|index)(?:$|[\s_\-/])/i.test(joined) && !/(montant|amount|valeur|total|depense|dépense|paiement|engagement|liquidation|budget|credit|crédit|solde|realisation|réalisation)/i.test(joined)) return true;
        return false;
    }

    // Filtrage des métadonnées techniques Kobo selon le rôle de chaque liste de l'interface.
    // IMPORTANT : on ne supprime PAS aveuglément tous les champs contenant "index".
    // Des champs métier comme activite_dren_index ou date_execution_index peuvent être utiles.
    function isTechnicalUiField(field, role = 'generic') {
        const raw = clean(field).toLowerCase();
        const leaf = raw.split('/').pop().replace(/^_+/, '');
        const label = clean(fieldLabel(field)).toLowerCase();
        const joined = `${raw} ${label}`;
        ...
        const exactTechnical = new Set([
            'row','__row','_row','id','_id','uid','_uid','uuid','_uuid',
            'instanceid','instance_id','submissionid','submission_id',
            'version','__version__','_version','xform_id_string','_xform_id_string',
            '_attachments','attachments','_notes','notes','_tags','tags',
            '_status','status','_validation_status','validation_status',
            '_submitted_by','submitted_by','_geolocation','geolocation'
        ]);
        if (exactTechnical.has(raw) || exactTechnical.has(leaf)) return true;
        if (raw.includes('formhub/uuid') || raw.includes('meta/instanceid')) return true;
        if (/^(?:meta\/|formhub\/)/i.test(raw) && role !== 'date') return true;

        // Identifiants/versions techniques explicites, sans éliminer les noms métier
        // tels que activite_dren_index / date_execution_index.
        if (/(^|[\/_-])(?:instance_?id|submission_?id|xform_?id|uuid|__version__)(?:$|[\/_-])/i.test(raw)) return true;

        if (role === 'category') {
            if (/(^|[\/_-])(?:version|validation_status|submitted_by|status)(?:$|[\/_-])/i.test(raw)) return true;
        }
        if (role === 'label' || role === 'semantic') {
            if (/(^|[\/_-])(?:version|validation_status|submitted_by|status|row)(?:$|[\/_-])/i.test(raw)) return true;
            // Un champ dont le libellé est manifestement une métadonnée Kobo n'est pas
            // proposé comme texte humain / sémantique.
            if (/(kobo|submission|instance|xform|uuid|version|validation)/i.test(joined) &&
                !/(libelle|libellé|intitule|intitulé|designation|désignation|description|objet|observation|commentaire|motif|beneficiaire|bénéficiaire)/i.test(joined)) return true;
        }
        if (role === 'reference') {
            if (/(^|[\/_-])(?:version|validation_status|submitted_by|status|row)(?:$|[\/_-])/i.test(raw)) return true;
        }
    ...
    function inferSchema(rows) {
        ...
        state.fields=fields; state.meta=meta; state.labelCompanions = new Map();
    }

    function detectField(patterns, types = null, role = 'generic') {
        const candidates = state.fields.map(field => ({field, meta:state.meta.get(field), lower:`${field} ${fieldLabel(field)}`.toLowerCase()}))
            .filter(x => (!types || types.includes(x.meta?.type)) && !isTechnicalUiField(x.field, role));
    ...
    function currentMappings() {
        return {
            ...
            reference: byId('exec-reference-field')?.value || '', label: byId('exec-label-field')?.value || '', semantic: byId('exec-semantic-field')?.value || '',
    ...
    function populateFieldControls() {
        ...
        const analyticalNumberFields=numberFields.filter(f=>!isTechnicalAnalysisField(f));

        // Listes nettoyées selon l'usage. On conserve les champs métier utiles même si leur
        // nom contient "index" (ex. activite_dren_index, date_execution_index).
        const dateFields=state.fields.filter(f=>['date','datetime'].includes(state.meta.get(f)?.type) && !isTechnicalUiField(f,'date'));
        const categoryFields=state.fields.filter(f=>state.meta.get(f)?.type==='category' && !isTechnicalUiField(f,'category'));
        const textFields=state.fields.filter(f=>['text','category'].includes(state.meta.get(f)?.type));
        const humanTextFields=textFields.filter(f=>!isTechnicalUiField(f,'label'));
        const semanticFields=textFields.filter(f=>!isTechnicalUiField(f,'semantic'));
        const referenceFields=state.fields.filter(f=>!isTechnicalUiField(f,'reference'));
        const entityFields=state.fields.filter(f=>!isTechnicalUiField(f,'entity'));

        const autoMeasure=analyticalNumberFields.find(f=>/(montant|amount|valeur|total|depense|dépense|paiement|engagement|liquidation|budget|credit|crédit|solde|realisation|réalisation)/i.test(`${f} ${fieldLabel(f)}`)) || '';
        const autoDate=detectField([/_submission_time/i,/(date.*heure|datetime|timestamp|date.*operation|date.*opération|date.*paiement|date.*engagement|date)/i],['date','datetime'],'date') || dateFields[0] || '';
        const autoEntity=detectField([/(dren|cisco|zap)/i,/(entite|entité|structure|service|unite|unité|direction|activite|activité|produit)/i],['category','text'],'entity') || categoryFields[0] || '';
        const autoRef=detectField([/(reference|référence|numero.*piece|num.*piece|pi[eè]ce|facture|mandat|engagement|bon.*commande|n°)/i],['text','category','number'],'reference') || '';
        const companionLabel=findCompanionLabelField(autoEntity);
        const autoLabel=(companionLabel && humanTextFields.includes(companionLabel) ? companionLabel : '') || detectField([/(libelle|libellé|label|intitule|intitulé|designation|désignation|nom.*activite|nom.*activité|description)/i],['text','category'],'label') || '';
        const autoSemantic=detectField([/(libelle|libellé|objet|description|justification|observation|commentaire|motif|intitule|intitulé|beneficiaire|bénéficiaire)/i],['text','category'],'semantic') || semanticFields[0] || '';

        fillSelect('exec-filter-text-field', humanTextFields, 'Tous les champs', '');
        ...
        fillSelect('exec-filter-number-field', analyticalNumberFields, 'Aucun champ', autoMeasure);
        fillSelect('exec-measure-field', analyticalNumberFields, 'Comptage des écritures', autoMeasure);
        fillSelect('exec-entity-field', entityFields, 'Détection automatique', autoEntity);
        fillSelect('exec-label-field', humanTextFields, 'Aucun libellé complémentaire', autoLabel);
        fillSelect('exec-reference-field', referenceFields, 'Détection automatique', autoRef);
        fillSelect('exec-semantic-field', semanticFields, 'Détection automatique', autoSemantic);
        ['exec-kmeans-x','exec-kmeans-y','exec-dbscan-x','exec-dbscan-y','exec-jenks-field','exec-stats-x','exec-stats-y','exec-forecast-value'].forEach((id,i)=>fillPureSelect(id,analyticalNumberFields,analyticalNumberFields[Math.min(i%2,Math.max(0,analyticalNumberFields.length-1))] || autoMeasure));
        fillPureSelect('exec-stats-group', categoryFields.length?categoryFields:entityFields, autoEntity || categoryFields[0]);
        fillPureSelect('exec-stats-group2', categoryFields.length?categoryFields:entityFields, categoryFields.find(f => f !== autoEntity) || categoryFields[0] || autoEntity);
    ...
    function renderOverview() {
        const rows=state.filtered, measure=byId('exec-measure-field')?.value || '', dateField=byId('exec-filter-date-field')?.value || '', entityField=byId('exec-entity-field')?.value || '', labelField=byId('exec-label-field')?.value || '';
        ...
        const measureLabel=measure?(state.meta.get(measure)?.label||measure):'Nombre d’écritures';
        const dateLabel=dateField?(state.meta.get(dateField)?.label||dateField):'Date / Heure';
        const entityLabel=entityField?(state.meta.get(entityField)?.label||entityField):'Entité / catégorie';
        ...
        let topText=''; if(entityField){const map=new Map();rows.forEach(r=>{const code=clean(r[entityField])||'Non renseigné',display=displayCodeAndLabel(r,entityField,labelField),v=measure?toNum(r[measure]):1;if(!map.has(code))map.set(code,{display,total:0});map.get(code).total+=(Number.isFinite(v)?v:0);});const top=[...map.values()].sort((a,b)=>b.total-a.total)[0];if(top)topText=` L'entité dominante est <strong>${esc(top.display)}</strong> (${fmtNumber(top.total)}).`;}
        byId('exec-narrative').innerHTML=`<strong>Lecture automatique :</strong> ${rows.length.toLocaleString('fr-FR')} écriture(s) sont retenues${measure?`, pour un total de <strong>${fmtNumber(vals.reduce((s,v)=>s+v,0))}</strong> sur « ${esc(measureLabel)} »`:''}. ${vals.length?`La moyenne est ${fmtNumber(mean(vals))}, la médiane ${fmtNumber(median(vals))} et le coefficient de variation ${fmtPct(cv)}.`:''} ${dateMin?`La période observée va du <strong>${fmtDate(dateMin)}</strong> au <strong>${fmtDate(dateMax)}</strong>.`:''}${topText} <strong>${outliers.length}</strong> valeur(s) sont atypiques selon le MAD robuste. La complétude des champs critiques sélectionnés est estimée à <strong>${fmtPct(q)}</strong>.`;

        if(dateField){
            const buckets=new Map();rows.forEach(r=>{const d=toDate(r[dateField]);if(!d)return;const key=d.toISOString().slice(0,10),v=measure?toNum(r[measure]):1;buckets.set(key,(buckets.get(key)||0)+(Number.isFinite(v)?v:0));});const data=[...buckets.entries()].sort((a,b)=>a[0].localeCompare(b[0]));
            chart('overview-time','exec-chart-time',{type:'line',data:{labels:data.map(x=>x[0]),datasets:[{label:measureLabel,data:data.map(x=>x[1]),tension:.2,pointRadius:3}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},title:graphTitle(`Évolution temporelle — ${measureLabel} selon ${dateLabel}`)},scales:{x:{title:{display:true,text:dateLabel},ticks:{maxRotation:60,minRotation:0}},y:{beginAtZero:true,title:{display:true,text:measureLabel}}}}});
        } else blankChart('overview-time','exec-chart-time','Sélectionnez un champ Date/Heure');
        if(entityField){
            const map=new Map();rows.forEach(r=>{const code=clean(r[entityField])||'Non renseigné',display=displayCodeAndLabel(r,entityField,labelField),v=measure?toNum(r[measure]):1;if(!map.has(code))map.set(code,{display,total:0});map.get(code).total+=(Number.isFinite(v)?v:0);});const data=[...map.values()].sort((a,b)=>b.total-a.total).slice(0,20);
            chart('overview-entities','exec-chart-entities',{type:'bar',data:{labels:data.map(x=>wrapChartLabel(x.display)),datasets:[{label:measureLabel,data:data.map(x=>x.total)}]},options:{responsive:true,maintainAspectRatio:false,indexAxis:'y',plugins:{legend:{display:false},title:graphTitle(`Principales ${entityLabel} — code et libellé`)},scales:{x:{beginAtZero:true,title:{display:true,text:measureLabel}},y:{title:{display:true,text:`${entityLabel} (code — libellé)`},ticks:{autoSkip:false}}}}});
        } else blankChart('overview-entities','exec-chart-entities','Sélectionnez un champ Entité / structure');
        if(vals.length){
            const min=Math.min(...vals),max=Math.max(...vals),bins=Math.min(20,Math.max(5,Math.round(Math.sqrt(vals.length)))),step=(max-min||1)/bins,counts=Array(bins).fill(0);vals.forEach(v=>counts[Math.min(bins-1,Math.floor((v-min)/step))]++);const labels=counts.map((_,i)=>`${fmtNumber(min+i*step,1)}–${fmtNumber(min+(i+1)*step,1)}`);
            chart('overview-hist','exec-chart-hist',{type:'bar',data:{labels,datasets:[{label:'Fréquence',data:counts}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},title:graphTitle(`Distribution de la mesure — ${measureLabel}`)},scales:{x:{title:{display:true,text:measureLabel},ticks:{autoSkip:true,maxRotation:60}},y:{beginAtZero:true,title:{display:true,text:'Fréquence / nombre d’écritures'}}}}});
        } else blankChart('overview-hist','exec-chart-hist','Aucune mesure comptable sélectionnée — les champs techniques comme « row » sont exclus');
        const critical=[currentMappings().measure,currentMappings().date,currentMappings().entity,currentMappings().reference].filter(Boolean);const labels=critical.map(f=>state.meta.get(f)?.label||f),rates=critical.map(f=>rows.filter(r=>!isEmpty(r[f])).length/rows.length*100);
        chart('overview-quality','exec-chart-quality',{type:'bar',data:{labels,datasets:[{label:'Complétude (%)',data:rates}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},title:graphTitle('Qualité et complétude — taux de renseignement des champs critiques')},scales:{x:{title:{display:true,text:'Champs critiques analysés'}},y:{beginAtZero:true,max:100,title:{display:true,text:'Complétude (%)'}}}}});
    }
    ...
    function runKMeans() {
        ...
        chart('kmeans','exec-chart-kmeans',{type:'scatter',data:{datasets:Array.from({length:k},(_,j)=>({label:`Cluster ${j+1}`,data:pts.map((p,i)=>assign[i]===j?{x:p.x,y:p.y,row:p.r}:null).filter(Boolean),pointRadius:5}))},options:{responsive:true,maintainAspectRatio:false,plugins:{title:graphTitle(`K-means — ${(state.meta.get(xf)?.label||xf)} × ${(state.meta.get(yf)?.label||yf)}`),tooltip:{callbacks:{label:c=>`#${c.raw.row.__row} : ${fmtNumber(c.raw.x)} ; ${fmtNumber(c.raw.y)}`}}},scales:{x:{title:{display:true,text:state.meta.get(xf)?.label||xf}},y:{title:{display:true,text:state.meta.get(yf)?.label||yf}}}}});
    ...
    function runJenks() {
        ...
        chart('jenks','exec-chart-jenks',{type:'bar',data:{labels:cls.map((_,i)=>`Classe ${i+1}\n${fmtNumber(br[i])}–${fmtNumber(br[i+1])}`),datasets:[{label:'Nombre d’écritures',data:cls}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},title:graphTitle(`Algorithme de Jenks — classes naturelles de ${state.meta.get(f)?.label||f}`)},scales:{x:{title:{display:true,text:'Classes naturelles'}},y:{beginAtZero:true,title:{display:true,text:'Nombre d’écritures'}}}}});byId('exec-jenks-table').innerHTML=tableHtml(['Classe','Minimum','Maximum','N','Part'],cls.map((n,i)=>[`Classe ${i+1}`,fmtNumber(br[i]),fmtNumber(br[i+1]),n,fmtPct(n/vals.length*100)]));
    }
    ...
    function runStatistics() {
        ...
        chart('stats-corr', 'exec-chart-correlation', {
            ...
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, title: graphTitle(`Corrélation Pearson / Spearman — ${(state.meta.get(xf)?.label || xf)} × ${(state.meta.get(yf)?.label || yf)}`) }, scales: { x: { title: { display: true, text: state.meta.get(xf)?.label || xf } }, y: { title: { display: true, text: state.meta.get(yf)?.label || yf } } } }
        });
        ...
        chart('stats-anova', 'exec-chart-anova', {
            ...
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, title: graphTitle(`ANOVA — moyenne de ${(state.meta.get(yf)?.label || yf)} par ${(state.meta.get(gf)?.label || gf)}`) }, scales: { x: { title:{display:true,text:state.meta.get(gf)?.label || gf}, ticks: { maxRotation: 70, minRotation: 30 } }, y: { beginAtZero: true, title:{display:true,text:`Moyenne de ${state.meta.get(yf)?.label || yf}`} } } }
        });
    ...
    function renderRisk() {
        ...
        chart('risk', 'exec-chart-risk', {
            ...
            options: {
                ...
                plugins: { legend: { display: false }, title: graphTitle('Anomalies & Risques — Top 30 des écritures selon le score de risque') },
                scales: { y: { beginAtZero: true, max: 100, title:{display:true,text:'Score de risque (0–100)'} }, x: { ticks: { autoSkip: false, maxRotation: 60 } } }
            }
        });
        renderBenford();
    ...
    function renderBenford() {
        const f=byId('exec-measure-field')?.value||'';
        if(!f){
            blankChart('benford','exec-chart-benford','Test de Benford — sélectionnez une vraie mesure comptable numérique');
            return;
        }
        const values=state.filtered.map(r=>Math.abs(toNum(r[f]))).filter(v=>Number.isFinite(v)&&v>0);const obs=Array(9).fill(0);values.forEach(v=>{const s=v.toExponential().replace('.','');const d=Number(String(v).replace(/^0\.?0*/, '').match(/[1-9]/)?.[0]||String(Math.floor(v))[0]);if(d>=1&&d<=9)obs[d-1]++;});const obsPct=obs.map(n=>values.length?n/values.length*100:0),exp=Array.from({length:9},(_,i)=>Math.log10(1+1/(i+1))*100);chart('benford','exec-chart-benford',{type:'bar',data:{labels:['1','2','3','4','5','6','7','8','9'],datasets:[{label:'Observé %',data:obsPct},{label:'Benford %',data:exp,type:'line',tension:.15}]},options:{responsive:true,maintainAspectRatio:false,plugins:{title:graphTitle(`Test de Benford — premier chiffre de ${state.meta.get(f)?.label||f}`)},scales:{x:{title:{display:true,text:'Premier chiffre significatif'}},y:{beginAtZero:true,title:{display:true,text:'Pourcentage'}}}}});
    }
    ...
    function wireEvents(){
        ...
        byId('exec-entity-field')?.addEventListener('change',()=>{const entity=byId('exec-entity-field')?.value||'',label=findCompanionLabelField(entity),el=byId('exec-label-field');if(el&&label&&[...el.options].some(o=>o.value===label))el.value=label;refreshAll();});
        ['exec-measure-field','exec-label-field','exec-reference-field','exec-filter-date-field'].forEach(id=>byId(id)?.addEventListener('change',refreshAll));
    ...
    function init(){
        ...
        loadExecutionDictionary().then(ok => { if (ok && state.raw.length) refreshAll(); });
