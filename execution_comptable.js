/* ============================================================================
   EXECUTION_COMPTABLE — V7.2 — 2026-08-28
...
(function () {
    ...
    function lookupDictionaryLabel(codeField, rawCode) {
        ...
        try {
            ...
            if (fallback && fallback.toLowerCase() !== codeKey) return fallback;
        } catch (_) {}
        return '';
    }
    function findCompanionLabelField(codeField) {
        ...
        for (const candidate of candidates) {
            ...
            if (a && b && a === b) score += 2;
            if (/(code|uid|identifiant|numero|num|produit|sous.?produit)/i.test(fieldLabel(candidate)) && !hasLabelWord) score -= 7;
            const paired = state.flat.slice(0,400).map(r => [clean(r[codeField]), clean(r[candidate])]).filter(([c,l]) => c && l && c !== l);
            if (paired.length) {
                const codeLikeRatio = paired.filter(([,l]) => isLikelyCodeValue(l)).length / paired.length;
    ...
    function displayCodeAndLabel(row, codeField, labelField = '') {
        ...
        // Un autre code (ex. P012) n'est jamais considéré comme le libellé de A3124.
        if (!label || label === code || isLikelyCodeValue(label)) label = dictionaryLabel || '';
        if (!label || label === code || code.includes(label)) return code;
